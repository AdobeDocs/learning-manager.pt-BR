...
As informações na tabela não são exaustivas. Consulte a seção **Lista de colunas em conjuntos de dados** para obter uma lista completa de todas as colunas em conjuntos de dados.

| **Conjunto de dados** | **O que ele contém** | **Campos de chave** |
|----------------------------------------------------------|--------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Usuário** | Dados de perfil do aluno para alunos ativos e excluídos | Nome, email, ID de usuário, gerente, campos ativos, status |
| **Transcrição (Objeto de Aprendizado) e Transcrição (Módulo)** | Registros de inscrição e conclusão | Data de inscrição, data de conclusão, percentual de andamento, status |
| **Objeto de Aprendizado** | Curso, caminho de aprendizado e metadados de certificação | Nome do objeto de aprendizado, ID do objeto de aprendizado, tipo, catálogo, rótulo do catálogo, status |
| **Instância do Objeto de Aprendizado** | Detalhes no nível da instância para cursos com várias instâncias | Nome da instância, ID da instância, limite de inscrição, prazo |
| **Catálogo** | Pares chave-valor de metadados de catálogo e etiquetas de catálogo | Nomes do autor, data de criação, duração, formato. **As colunas de Etiqueta de Catálogo estão configuradas pelo cliente**. Elas aparecem somente se sua conta tiver etiquetas de catálogo configuradas. Os nomes e valores de rótulos exibidos refletirão sua própria configuração. |
| **Grupo de usuários** | Associação e hierarquia de grupos de usuários | Data de criação, contagem de membros, Nome, Status |
| **Módulo** | Detalhes da sessão para módulos de sala de aula, sala de aula virtual e e-learning | ID do módulo, nome(s) do professor, hora de início, hora de término, local |


>[!NOTE]
>
>Para cursos adquiridos de outra conta por meio do compartilhamento de catálogo, a coluna **Nomes de autores** no conjunto de dados do Objeto de aprendizado retorna o nome do autor original da conta de origem. Isso difere da interface padrão do administrador do ALM e dos relatórios fixos, em que os cursos adquiridos exibem “Autor externo”. Esse comportamento é específico do Report Builder e se aplica somente a contas de mesmo nível (de recebimento).
...
>[!NOTE]
>
>Se um relatório gerado contiver dados do curso, mas não dados do caminho de aprendizado, verifique se você ingressou no conjunto de dados do Objeto de aprendizado com o conjunto de dados do módulo. Os módulos são vinculados diretamente aos cursos, mas não diretamente aos caminhos de aprendizado. Como resultado, quando os campos Módulo são incluídos, o Report Builder pode retornar apenas registros do curso e filtrar caminhos de aprendizado. Para gerar relatórios sobre caminhos de aprendizado, evite adicionar campos de Módulo, a menos que o relatório pretenda especificamente analisar os dados do módulo no nível do curso.
...
* **Registro** (conjunto de dados de registro), a tabela de fatos primária. Ele se conecta diretamente ao **Usuário** (o aluno que se inscreveu), ao **Objeto de Aprendizado** (no que se inscreveu) e por meio do objeto de aprendizado ao **Módulo**, ao **Catálogo**, à **Etiqueta de Catálogo** e à **Instância do OA**.
* **Transcrição do módulo** (conjunto de dados moduleTranscript) - a tabela de fatos de progresso no nível do módulo. Ele se conecta ao **Usuário** e ao **Módulo**, que, por sua vez, vincula ao **Objeto de Aprendizado**.
...
* **Professor**: o módulo ingressa no usuário como professor da sessão
* **Autor**: o autor do objeto de aprendizado ingressa no usuário como autor do objeto de aprendizado
...
### Tipos de conjunto de dados de grupo de usuários
...
| **Conjunto de dados** | **Tipo de grupo** |
|---------------------------------------------------------|---------------------------------------------------------------|
| **Grupo de Usuários** (userGroup) | Todos os grupos de usuários. Usar este conjunto de dados como o grupo de usuários principal |
| **Grupo de Usuários (Campo Ativo)** (ative_field_user_group) | Grupos com base em valores de campo ativos (por exemplo, região, departamento) |
| **Grupo de usuários (Equipe)** (team_user_group) | Grupos baseados em hierarquia de gerentes |
| **Grupo de usuários (Personalizado)** (custom_user_group) | Grupos personalizados configurados manualmente |
| **Grupo de usuários (interno)** (inbuilt_user_group) | Grupos definidos pelo sistema |
...
| **Campo derivado** | **O que ele calcula** | **Conjuntos de dados necessários** |
|-------------------------|-------------------------------------------------------------------------|-----------------------------------------|
| **Conformidade %** | Porcentagem de alunos necessários que concluíram cursos marcados como de conformidade | Grupo de usuários, Objeto de aprendizado, Transcrição |
| **Conclusão %** | Conclusões divididas por inscrições x 100 | Transcrição ou Objeto de aprendizado |
| **Contagem de Usuários Inscritos** | Total de alunos inscritos em um objeto de aprendizado | Objeto de aprendizado |
| **Contagem de Conclusão** | Total de conclusões de um objeto de aprendizado | Objeto de aprendizado |
| **Iniciar Contagem** | Alunos que iniciaram, mas não concluíram | Objeto de aprendizado |
| **Contagem de Membros** | Número de usuários em um grupo de usuários | Grupo de usuários |
