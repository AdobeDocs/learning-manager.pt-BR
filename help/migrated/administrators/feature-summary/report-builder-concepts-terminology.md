...
**Relatórios** são suas próprias configurações salvas. Você pode criar um relatório do zero ou duplicando um modelo e editando a cópia. Quando você duplica um modelo, a cópia torna-se um relatório na guia **Relatórios**. Os modelos e relatórios são exibidos em Report Builder, mas sob guias separadas.
...
* **Módulo**: dados da sala de aula e da sessão virtual, incluindo detalhes do módulo de aprendizado eletrônico
...
Agrupar por segue o comportamento padrão do banco de dados: depois que você aplicar agrupar por em uma coluna, todas as outras colunas do relatório deverão ter uma função agregada aplicada. Você não pode misturar dados de linha individuais com dados agrupados. As funções agregadas disponíveis são:

* **Contagem**: número total de linhas
* **Contar se**: número de linhas em que o campo corresponde a um valor especificado
* **Soma**: total de um campo numérico
* **Min**: menor valor em um campo numérico
...
Vários filtros assumem como padrão a lógica AND; todas as condições devem ser verdadeiras para que uma linha seja exibida. Você pode alternar o operador entre dois filtros quaisquer para OU. Você também pode agrupar filtros usando **Adicionar como grupo**, que cria um colchete. Os filtros dentro do grupo são avaliados juntos antes de serem combinados com os filtros fora dele.

Isso permite criar condições como: (catálogo = Segurança OU catálogo = Higiene) E a data de conclusão está nos últimos 90 dias.
...
O Report Builder oferece suporte à inclusão de alunos excluídos em relatórios e à recuperação de seus valores de campo ativos.

Use a coluna Data de Exclusão do conjunto de dados Usuário para criar o relatório.
