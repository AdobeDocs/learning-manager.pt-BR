...
**Widget Salvo por mim**

* **Administrador:** os administradores podem colocar o widget Salvo por mim em uma página para que os alunos possam encontrar facilmente o conteúdo marcado. Isso permite que os alunos retornem aos cursos salvos anteriormente sem precisar pesquisá-los ou navegar até eles novamente. Saiba mais sobre o widget [Salvo por mim](/help/migrated/administrators/feature-summary/experience-builder/add-a-widget.md#bookmark-widget).

* **Aluno:** o widget **Salvo por mim** mostra cursos, programações de aprendizado, certificações e ajudas de tarefa que você marcou para mais tarde. Use esse widget para acessar rapidamente o conteúdo salvo em um só lugar, sem precisar pesquisar o catálogo novamente. Saiba mais sobre o widget [Salvo por mim](/help/migrated/learners/feature-summary/saved-by-me-widget.md).

**Cor do tema do canal:** ao criar um canal, você tem mais cores do tema do canal para escolher. As cores do tema são contextualmente aplicadas para destacar o contraste após a seleção. Quando você seleciona uma cor de tema para o canal, a cor selecionada é aplicada à interface do canal e é projetada para manter o contraste com miniaturas e texto do canal. Uma cor padrão é atribuída automaticamente quando você cria um canal, mas é possível escolher uma cor diferente nas opções disponíveis a qualquer momento.

**Miniaturas de vídeo:** agora o aluno pode identificar rapidamente um vídeo por seu bloco, que, por sua vez, exibe uma miniatura para ajudar a identificar seu conteúdo rapidamente. Se o vídeo de origem não fornecer sua própria imagem em miniatura, o Adobe Learning Manager gerará uma automaticamente usando a cor do tema do canal, um padrão abstrato e o título do vídeo.

**Canais:** a opção de navegação Canais é visível para os alunos somente quando pelo menos um canal foi criado, contém pelo menos um vídeo de sua origem e está marcada como Visível para os alunos. Saiba mais sobre [Canais](/help/migrated/administrators/feature-summary/create-channels.md).

## Erros corrigidos

**Componente do Adobe Experience Manager:** após atualizar para o pacote do Site de Referência da Adobe Learning Manager v2.0.1, o painel Notificações não foi carregado quando os usuários clicaram no sino de notificações. O carregador continuou girando indefinidamente, impedindo que as notificações fossem exibidas. O problema também causou chamadas repetidas ao endpoint /userNotifications, resultando em respostas HTTP 429. O fluxo de trabalho de carregamento de notificação foi corrigido para evitar solicitações excessivas de API e resolver o problema de carregamento. As notificações agora são carregadas corretamente, e o painel é aberto conforme esperado sem limitação desnecessária da API.

**Aplicativo móvel do Adobe Learning Manager > Caminho de aprendizado:** os cursos concluídos em um caminho de aprendizado no aplicativo móvel do Adobe Learning Manager eram exibidos incorretamente com um indicador amarelo e um ponto vermelho, mesmo que o status estivesse marcado como concluído. Essa inconsistência visual ocorreu somente nos cursos dentro dos Caminhos de aprendizado, enquanto os módulos concluídos eram exibidos corretamente. A lógica do indicador de status foi corrigida para os cursos do Caminho de Aprendizado no aplicativo móvel. Os cursos concluídos agora exibem o indicador de conclusão verde correto. Isso garante uma experiência consistente de conclusão no aplicativo móvel e no navegador de desktop.

+++


+++Atualização 110: versão de 16 de agosto de 2026 do Adobe Learning Manager

## Liberar destaques

A versão de agosto de 2026 do Adobe Learning Manager oferece avanços significativos em aprendizado personalizado, IA, relatórios e integrações.

O gradiente repleto de recursos adiciona pontuação ponderada e agrega o cálculo de aprovação/reprovação aos cursos, dando às organizações uma medida precisa e configurável do desempenho do aluno, além do rastreamento da conclusão simples.

No plano de IA, o Assistente de IA para alunos recebe resumos de cursos, comparação de objetos de aprendizado, respostas do Adobe Experience League e consultas de conteúdo de terceiros dos catálogos Go1 e LinkedIn Learning, um novo agente do Caminho de aprendizado orienta os alunos por meio de uma conversa para gerar automaticamente um caminho de aprendizado sequenciado personalizado e o Agente do Insights traz a consulta de linguagem natural para os relatórios do administrador. Todos os recursos de IA são regidos por um novo sistema de créditos Gen AI na página Faturamento, fornecendo aos administradores limites de uso por recurso e alertas de consumo.

O gerenciamento de conteúdo é fortalecido com um criador de modelos de e-mail baseado em componentes WYSIWYG, uma estrutura de pastas de conteúdo hierárquico com acesso baseado em funções, um recurso de aprendizado externo para enviar e aprovar treinamento fora da plataforma e um novo recurso de canais que agrega conteúdo de vídeo de páginas da Web corporativas e páginas de conferência.

Saiba mais sobre as [Novidades do lançamento do Adobe Learning Manager em agosto de 2026](/help/migrated/whats-new.md).

## Alterações interruptivas na versão

### Recurso de aprendizado externo: novas colunas na transcrição do aluno

Eles adicionam campos personalizados de Aprendizado externo específicos da conta ao relatório LT. No LT do administrador, novas colunas de campo personalizado são adicionadas ao final; no LT do aluno, o nome do aprendizado externo e o comentário de conclusão são inseridos perto dos campos relacionados do módulo e do revisor, com os campos personalizados restantes adicionados no final.
...
### ID de treinamento raiz para certificações na transcrição do aluno

Isso adiciona uma nova coluna ID de treinamento raiz ao LT de administrador e ao LT de aluno. A coluna aparece no final do relatório e ajuda a identificar o contexto de certificação pai do registro do aluno.

Saiba mais sobre as [alterações de relatórios na versão de agosto de 2026 do Adobe Learning Manager](/help/migrated/reporting-changes-august-2026.md).

### Alinhamento do formato de data-hora do Webhook com a transcrição do aluno

Isso padroniza os valores de data-hora do objeto de dados do webhook para precisão de minuto. Os segundos são sempre emitidos como 00, alinhando os carimbos de data/hora do webhook com o formato de relatório LT.

Saiba mais sobre as [alterações de relatórios na versão de agosto de 2026 do Adobe Learning Manager](/help/migrated/reporting-changes-august-2026.md).

### Coluna de espessura na transcrição do aluno

Uma coluna Espessura é adicionada ao relatório LT para módulos em cursos habilitados para Gradebook. Isso expõe o peso do módulo diretamente na saída do relatório.

Saiba mais sobre as [alterações de relatórios na versão de agosto de 2026 do Adobe Learning Manager](/help/migrated/reporting-changes-august-2026.md).

### Detalhes do autor do curso compartilhado na API learningObjects

Os detalhes atualizam a resposta do OA da API learningObjects para cursos compartilhados, de modo que as contas de recebimento não mostrem mais o administrador de aceitação como autor. Somente os detalhes do autor externo original são expostos em contas entre parceiros; o comportamento da conta principal permanece inalterado.

### Detecção de intenção no Agente do AI Orchestrator

O Agente do AI Orchestrator move a detecção de intenção para solicitações de consulta única para o orquestrador e remove os alternadores manuais da Interface do usuário. O roteamento de consultas pode mudar entre os fluxos de Perguntas e Respostas e PLP, dependendo da intenção detectada.

## Erros corrigidos na versão

**Transcrição do aluno:** as datas no Relatório de transcrição do aluno eram usadas para registrar 00 por segundos, enquanto no webhook os segundos apareciam como deveriam - em dois dígitos. Esse problema foi corrigido. A transcrição do aluno agora também mostra corretamente os segundos de dois dígitos.

**Instância:** quando uma sessão Microsoft Teams ou Adobe Connect VC foi cancelada na página da instância, alguns detalhes da sessão associada permaneceram após uma atualização da página. Esse problema foi corrigido e as sessões canceladas agora removem corretamente todos os detalhes associados, incluindo as configurações do organizador de equipes e de desvio da sala de espera e o professor primário do Connect.

**API:** um problema em um fluxo de trabalho automatizado do AWS fez com que solicitações repetidas de download para a API HTTP do CMS usassem URLs codificadas incorretamente, resultando em erros HTTP 500 intermitentes e grandes volumes de tráfego inválido. O fluxo de trabalho foi atualizado para codificar corretamente espaços e outros caracteres especiais em URLs de solicitação de download. Como resultado, as solicitações de download de ativos da conta são processadas com êxito, reduzindo erros e aumentando a confiabilidade da recuperação automatizada de ativos.

**Aluno:** um problema fez com que a formatação em itálico aplicada na seção Visão geral detalhada de um curso fosse exibida corretamente durante a criação, mas não na página de visão geral do curso voltada para o aluno após a publicação. A lógica de renderização foi atualizada para preservar a formatação em itálico do texto quando o conteúdo do curso é publicado. Como resultado, os itálicos aplicados na seção Visão geral detalhada são exibidos de forma correta e consistente para os alunos na página de visão geral do curso publicado.

**Certificações personalizadas:** a visualização do certificado exibiu um fundo branco quando uma imagem de SVG foi carregada como fundo do certificado para um certificado de conquista. O processo de renderização da visualização do certificado foi atualizado para oferecer suporte e exibir corretamente as imagens de fundo de SVG carregadas. Como resultado, as visualizações do certificado agora renderizam o plano de fundo do SVG configurado conforme esperado, fornecendo uma representação precisa do certificado final.

**Autor:** a geração de arquivos VTT para vídeos interativos longos pode sofrer atrasos significativos, com tempos de processamento excedendo duas horas para alguns carregamentos. Esse problema foi investigado e otimizado para melhorar o desempenho e a confiabilidade da geração de VTT para arquivos de vídeo grandes. Como resultado, os arquivos VTT agora são gerados com mais eficiência, fornecendo um fluxo de trabalho de legendas mais rápido e consistente para uploads de vídeo interativo.

**API:** alguns registros de conclusão de aluno não estavam sendo retornados pela API de Exportação de Relatório, mesmo que os dados de conclusão existissem no back-end, correspondessem ao intervalo de datas especificado e aos filtros de grupo de usuários e atendessem a todos os critérios de relatório. O processo de exportação de relatório para conclusões do aluno em várias tarefas e na lógica de recuperação de dados foi aprimorado para garantir que os registros de conclusão qualificados sejam incluídos consistentemente nas exportações de API. As respostas da API de exportação de relatório agora retornam dados completos de conclusão do aluno para usuários cujos carimbos de data de conclusão estão dentro do intervalo de datas e do escopo do filtro solicitados, fornecendo resultados de relatórios mais precisos e confiáveis.

**Webhook:** quando um aluno se inscreveu em uma sessão que foi cancelada posteriormente, a inscrição em outra sessão do mesmo curso não acionava um novo evento de inscrição no webhook, a menos que o aluno fosse cancelado pela primeira vez na sessão cancelada. Isso fez com que os sistemas downstream perdessem atualizações de registro válidas para instâncias canceladas ou concluídas anteriormente. O fluxo do webhook foi atualizado para que um novo evento de inscrição seja acionado quando um aluno se inscrever em outra sessão do mesmo curso após a sessão anterior ser cancelada ou concluída, garantindo que a atividade de inscrição seja relatada de forma consistente.

**API:** as solicitações da API do administrador para caminhos de aprendizado que incluíam muitos dados relacionados (como instâncias, inscrições, recursos e graus) sofreram uma degradação significativa no desempenho, com os tempos de resposta aumentando dos típicos cinco para oito segundos para até 45 para 60 segundos, afetando os fluxos de trabalho de integração e os sistemas downstream. As otimizações de desempenho foram implementadas para incluir solicitações pesadas de objetos de aprendizado para melhorar a eficiência da recuperação de dados. As chamadas de API do Caminho de aprendizado que incluem relações aninhadas complexas agora retornam resultados de forma mais consistente e com tempos de resposta aprimorados, ajudando a garantir um desempenho de integração confiável.
...
**API:** quando os alunos classificavam o catálogo por nome em ordem crescente (A-Z), a API pública retornava resultados usando a classificação com distinção entre maiúsculas e minúsculas, fazendo com que os nomes dos cursos em maiúsculas (AA, BB, CC) aparecessem antes dos nomes em minúsculas (aa, bb) em vez de seguir a ordem alfabética padrão. A lógica de classificação foi atualizada para usar a comparação sem diferenciação de maiúsculas e minúsculas. Os resultados do catálogo agora são exibidos na sequência alfabética esperada, independentemente da capitalização das letras, garantindo uma ordem consistente dos nomes dos cursos.

**CR-VC:** quando um novo professor era adicionado a um curso de sala de aula virtual, nenhum email de convite de sessão era enviado se o modelo de email “Você foi adicionado como professor/organizador/coorganizador” estivesse desabilitado, impedindo que os professores recebessem detalhes da sessão. A lógica de notificação foi corrigida para garantir que os convites de sessão do professor sejam gerados independentemente do modelo.

**Cliente comum:** quando a duração de um módulo de atividade era atualizada após o curso ter sido adicionado a um plano de aprendizado AET (Atom Enrollment Trigger), a visualização do aluno continuava a exibir a duração da instância AET em vez da duração atualizada da instância padrão do curso. Esse problema foi corrigido garantindo que a visualização do aluno recuperasse a duração do módulo da instância correta do curso. As visualizações do aluno agora exibem a duração mais recente do módulo publicado com precisão, refletindo as atualizações feitas no conteúdo do curso.

**Aluno:** em uma certificação ordenada, os alunos podiam ignorar um primeiro curso com falha e acessar um segundo curso bloqueado preenchendo seu pré-requisito, permitindo que a certificação fosse marcada como concluída quando o requisito fosse definido como qualquer curso. A validação foi atualizada para aplicar as regras de bloqueio e ordem de cursos de forma consistente. Os alunos agora podem concluir os requisitos de certificação apenas na sequência definida, impedindo que os cursos bloqueados contribuam para a conclusão da certificação.

**API:** quando um recurso foi adicionado a um curso sem uma descrição, a API GET /learningObject/{id} não retornava uma descrição recém-adicionada se o recurso foi atualizado posteriormente. Isso resultou em metadados de recurso obsoletos sendo expostos por meio da API. O problema de sincronização foi corrigido e a API agora retorna a descrição de recurso mais recente, independentemente de quando foi adicionada.

**API:** quando um módulo foi migrado com uma descrição e a descrição foi atualizada posteriormente, o valor atualizado foi salvo corretamente na tabela do módulo, mas não foi refletido na interface do usuário. A interface do usuário continuou a exibir a descrição mais antiga porque estava sendo originada do registro content_group, que não estava sendo atualizado durante a modificação. Esse problema de sincronização foi resolvido, e as descrições atualizadas do módulo agora são refletidas de forma consistente na interface do usuário após a migração.

**Modelos de Email e Configuração:** quando os administradores atualizaram o banner de email nas Configurações de Modelo de Email, o novo banner apareceu corretamente no Outlook Desktop, mas não foi exibido no Outlook Web, resultando em uma experiência de email inconsistente entre os clientes. A renderização do modelo de email foi atualizada para garantir a compatibilidade do banner nas plataformas do Outlook. Os banners de email atualizados agora são exibidos corretamente nas versões do Outlook para desktop e do Outlook para Web.

## Problemas conhecidos na versão

### O professor não pode ingressar na sala de aula virtual entre módulos

Os professores não podem ingressar na sala de aula virtual ao tentar entrar em uma sala fora do horário agendado de seu módulo de curso atribuído.

O Adobe Learning Manager usa uma única sala de aula virtual para todos os módulos de um curso. Quando um professor inicia a sala, a sessão é associada ao módulo vinculado ao ingresso desse professor
URL. Qualquer solicitação de associação subsequente deve corresponder ao módulo ativo associado à sala. Se outro professor tentar ingressar usando o link de ingresso de um módulo diferente enquanto um módulo diferente estiver ativo, acesse o
sala negada. Somente um módulo pode estar ativo na sala compartilhada por vez.

Esse comportamento é esperado na versão atual. As melhorias para permitir que os professores do curso ingressem na sala independentemente do módulo ativo estão planejadas para uma versão futura.

### O áudio é interrompido quando o microfone e o alto-falante diferem no macOS

O áudio pode quebrar ou soar distorcido intermitentemente durante uma sessão de sala de aula virtual no macOS quando o microfone e o alto-falante selecionados forem dispositivos diferentes.

Esse comportamento pode ocorrer quando diferentes dispositivos são usados para entrada e saída de áudio, como AirPods para o microfone e alto-falante integrado para reprodução. Como cada dispositivo introduz seu próprio atraso de áudio, o eco
o cancelamento é menos eficaz e a supressão do ruído de fundo pode, ocasionalmente, identificar incorretamente partes da fala como ruído. Isso pode resultar em breves interrupções de áudio. O problema é mais perceptível quando a
a voz do alto-falante é capturada em um volume baixo, como quando o microfone é posicionado mais longe.

Esta é uma limitação de plataforma conhecida e não é específica do Adobe Learning Manager. Um comportamento semelhante foi observado em outros aplicativos de conferência. Nenhuma correção está incluída nesta versão e o problema está sendo avaliado para uma versão futura.

Use o mesmo dispositivo para microfone e alto-falantes para evitar esse comportamento.

### O vídeo compartilhado congela no Chrome após perder o foco

Quando um professor compartilha uma janela do Chrome reproduzindo um vídeo e depois muda o foco para longe dele, os participantes podem ver o conteúdo compartilhado congelado ou exibir uma tela preta.
...
Esta é uma limitação do navegador e do sistema operacional e não é específica do Adobe Learning Manager. Nenhuma correção específica do produto está disponível para esta versão.

Mantenha a janela do navegador compartilhado em foco ao apresentar o conteúdo do vídeo. Evite alternar de volta para a janela Sala de aula virtual durante a reprodução.

### O professor não pode ingressar em um módulo que já está em uso

Quando os professores tentam iniciar dois módulos do curso que usam a mesma sala de aula virtual ao mesmo tempo, apenas o primeiro professor pode entrar com sucesso na sala.

O Adobe Learning Manager usa uma única sala de aula virtual compartilhada para uma instância do curso. Quando o primeiro professor entra na sala, a sala é associada ao módulo desse professor. As solicitações de ingresso adicionais são
validado em relação ao módulo ativo. Se outro professor tentar ingressar usando um link de módulo diferente, o acesso será negado. Como resultado, as sessões simultâneas lideradas por instrutor não podem ser conduzidas na mesma
instância do curso usando módulos separados que compartilham uma sala.

Esse comportamento é esperado na versão atual e é consistente com a arquitetura da sala existente. A funcionalidade está sendo revisada para aprimoramento futuro com base no feedback do cliente.

### O conteúdo compartilhado exibe um efeito de túnel após retornar à janela do Live Hub

Quando um professor compartilha toda a tela e retorna à janela do Live Hub, os participantes podem experimentar um efeito de túnel no conteúdo compartilhado. Quando você compartilha sua tela inteira, o feed compartilhado inclui tudo o que é exibido em seu desktop. Alternar de volta para a janela do Live Hub pode fazer com que o conteúdo compartilhado capture e exiba repetidamente a interface do Live Hub, resultando em um comportamento visual inesperado. Essa é uma limitação conhecida do compartilhamento em tela cheia e está sendo avaliada para uma versão futura.

Compartilhe um aplicativo ou janela específica em vez de toda a tela. Por exemplo, compartilhe a apresentação ou janela do documento que você deseja que os participantes visualizem. Compartilhar um único aplicativo mantém o conteúdo compartilhado isolado e impede o efeito de túnel.

### O relatório do painel da sessão não estará disponível imediatamente após reingressar em uma sessão encerrada

O relatório do Painel de Sessão não estará disponível imediatamente após reingressar em uma sessão encerrada
Se um professor exibir o Painel de sessão após o término de uma sessão e depois reingressar na mesma sessão usando seu URL, o relatório existente do Painel de sessão pode não estar mais disponível. O relatório pode levar aproximadamente de cinco a sete minutos para ser atualizado e ficar disponível novamente enquanto a instância da sessão anterior é processada. Esta é uma limitação conhecida e está sendo avaliada para uma versão futura.

Aguarde de cinco a sete minutos após fechar a sessão antes de reabrir o URL do painel. Isso permite que a instância da sessão anterior conclua o processamento, após o qual o relatório do Painel da Sessão fica disponível novamente.

### Os nomes das colunas não estão localizados no CSV de locais de sala de aula exportado

Quando o idioma da interface do usuário é definido para um idioma diferente do inglês, o arquivo CSV exportado da página Locais de sala de aula exibe os nomes das colunas (a linha de cabeçalho) em inglês, e não no idioma selecionado.

Esse comportamento ocorre durante a exportação de Locais de sala de aula do perfil do administrador > Configurações > Locais de sala de aula. Embora os dados do local no arquivo sejam retornados corretamente, os cabeçalhos das colunas não são convertidos para corresponder ao local da interface do usuário escolhido pelo administrador. Como resultado, um administrador que trabalha em um local diferente do inglês vê os nomes de colunas em inglês em um ambiente localizado.

Somente a linha de cabeçalho é afetada; os dados do local subjacente no arquivo exportado não são afetados. Nenhuma correção está incluída nesta versão e o problema está sendo avaliado para uma versão futura.

### Entrada de limites do campo Duração do Crédito em 9999,99

Quando um usuário insere um valor de Duração de crédito maior que 9999,99, como 999999, no formulário de edição de sessão, o valor é limitado silenciosamente a 9999,99 ao salvar, sem nenhum erro de validação ou aviso mostrado ao usuário. O campo tem um atributo HTML max definido como 9999.99, que impõe esse limite no nível do navegador sem notificar o usuário de que sua entrada foi rejeitada ou modificada. Nenhuma correção foi aplicada para esta versão, pois esse comportamento foi confirmado como funcionando corretamente.

### A mensagem de erro não indica uma linha de idioma padrão ausente nos uploads de CSV

Quando um upload de CSV de local estruturado contém linhas apenas em localidades não padrão, como fr,de, e nenhuma linha no idioma padrão da conta, a mensagem de erro retornada é Locale no CSV, linha 1, e a coluna 2 é inválida ou não tem suporte. Isso não indica que a ocorrência real seja uma linha de idioma padrão ausente. Isso impede que os usuários carreguem CSVs de vários locais e diagnostiquem o problema apenas com mensagens de erro. Esse problema foi removido nesta versão sem nenhuma correção aplicada.

### O campo Limite de vagas permite valores irrealistas de até 4.294.967.295

O campo Limite de vagas no modal Local da sala de aula aceita qualquer valor de até 4.294.967.295 (~4,3 bilhões), um limite que é tecnicamente orientado pelo tipo de dados subjacente em vez de uma restrição de negócios realista. Os usuários podem inserir e salvar um número irreal de licenças, como 1.000.000.000, sem que nenhuma validação de nível empresarial os rejeite.
Nenhuma correção foi aplicada para esta versão; um valor máximo comercial realista ainda não foi acordado nem aplicado.
...
Para obter mais informações, consulte [Criando uma instância](/help/migrated/administrators/feature-summary/courses.md#createinstanceofacourse) para o Administrador e [Criar uma instância](/help/migrated/authors/feature-summary/courses.md#createaninstance) para o Autor.

**Email de Convite de Sessão:** quando um usuário é adicionado como professor para um tipo de sessão CR/VCR, o professor recebe um email de notificação automático. A aparência deste e-mail foi aprimorada esteticamente. Para obter mais informações, consulte [Convites do calendário](/help/migrated/instructors/feature-summary/learners.md#calendarinvites)

## Alterações de API

### Iniciar disponibilidade na API getLO
...
Para obter mais informações, [consulte Alterações de API na versão de maio](/help/migrated/api-changes-alm-may.md).
...
**Substituição de VTT bloqueada pelo limite de caracteres:** corrigido um problema em que o usuário não conseguia substituir o arquivo VTT gerado automaticamente pelo ALM devido ao limite de caracteres no modo de revisão/edição. O limite de caracteres foi aumentado para um milhão no modo de edição VTT.
...
**Data de lançamento:** 16 de março de 2024
...
+++Atualização 95: A versão de novembro de 2023 do Adobe Learning Manager
...
* Renomeamos a cadeia de caracteres Classificar este treinamento para Classificar este curso, Classificar este caminho de aprendizado ou Classificar esta certificação, com base no treinamento que o aluno faz. Dependendo do tipo de treinamento, um aluno vê a sequência de caracteres de acordo.
...
* Quando um CSV com um grande número de registros está em processo, leva um tempo significativo, durante o qual, se um usuário fizer logon e inserir um valor para um dos atributos, ele poderá criar um novo grupo de usuários que poderá resultar em erros de CSV. Para corrigir isso, quando a importação de CSV estiver em andamento, a mensagem pop-up do atributo Campos ativos será desativada e reativada assim que o upload do CSV for concluído.
...
* Convites de sessão que se estendem por vários dias, nos quais os convites não refletem as informações corretas sobre os dias que são bloqueados em alguns clientes de e-mail. Esse problema já foi corrigido.
...
O Laerning Manager oferece um painel de conformidade para todos os administradores e gerentes. Os gerentes acham muito útil controlar a conformidade dos membros da equipe para um treinamento específico. Ao mesmo tempo, os administradores gostariam que todos os gerentes adicionassem treinamentos de conformidade ao painel e os monitorassem.

No Learning Manager, o fluxo de trabalho **Compartilhar com Gerentes** permite que os administradores compartilhem treinamentos com gerentes, para que eles possam ser adicionados ao Painel de Conformidade de um gerente. Assim, os gerentes não precisam executar nenhuma ação e podem começar a monitorar a conformidade imediatamente.
...
* Se você adicionar vários catálogos a uma função personalizada, ocorrerá um erro ao atualizar a função. O limite de número de catálogos foi aumentado em até 50 catálogos.
...
* Obtenção e armazenamento externo de e-mails/tokens do usuário para que os usuários possam implementar os seus próprios back-ends de armazenamento em vez de usar nós de usuário da AEM.
...
* Um upload de CSV falhava se os campos ativos contivessem caracteres especiais. Esse problema foi corrigido.
...
* No **Aplicativo do administrador > Catálogo > procurar catálogo**, não era possível pesquisar.
...
* O instrutor recebia convites de sessão para todas as suas sessões, mesmo que apenas uma nova sessão fosse adicionada. Esse problema foi corrigido.
...
* Uma medalha personalizada com um rótulo de catálogo com um espaço não permite que o pdf seja baixado conforme esperado.
...
* Para identificar todos os cursos nos quais cada aluno está inscrito e se ele o concluiu, inclua os seguintes campos no painel, Relatório de assinatura:
...
Nesta atualização, você também pode alterar a imagem do banner para um Objeto de aprendizado.
...
* Na caixa de diálogo **Adicionar relatório**, os seletores de **Intervalo de tempo** e **Filtros** aparecem como seções separadas, que estão no estado contraído, por padrão. Para obter mais informações, consulte [***Criar relatórios***](../administrators/feature-summary/reports.md#report).
...
Nas transcrições do aluno, nas colunas de tempo, os minutos são arredondados até o minuto mais próximo e o valor do segundo é 00. Para obter mais informações, consulte [***Colunas de tempo***](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md).

### Problemas corrigidos nesta versão {#Issuesfixedinthisrelease-3}

**Painel do aluno**

* Um calendário de aprendizagem exibia o status **Inscrito na sessão** mesmo quando um gerente ainda não tinha aprovado a inscrição. Agora o status correto **Pendente** é exibido ao aluno até o gerente aprovar a inscrição.

* Em um caso específico, em uma sessão, o calendário de aprendizagem exibia o status **Inscrito** mesmo se o aluno tivesse concluído o curso.
...
Compartilhe imagens, documentos ou arquivos de áudio ou vídeo em qualquer painel, de forma que outros membros do painel possam visualizar sua publicação e começar uma interação.  Para obter mais informações, consulte [***Compartilhar publicação***](../learners/feature-summary/ipad-android-tablet-users.md#socialmobile).
...
Mapeie as habilidades da conta com os domínios de habilidades oferecidos no LMS do Learning Manager. Isso ajuda a vincular as habilidades da conta com os domínios de habilidades com suporte do Learning Manager para curadoria autoassistida. Para obter mais informações, consulte [***Mapear habilidade com domínios***](../administrators/feature-summary/curation-skills.md).
...
* Se o gerente selecionasse a opção **Neste trimestre**, os detalhes de inscrição, andamento e conclusão do objeto de aprendizado não eram exibidos. Nesta versão, os detalhes são exibidos como esperado.
...
* As habilidades agora oferecem suporte à concessão de valores de crédito no formato decimal. Isso permite que os autores concedam valores decimais de crédito a um determinado curso. Para obter mais informações, consulte [***Suporte a casas decimais***](../administrators/feature-summary/skills-levels.md#decimal).
...
* Baixe as transcrições de aluno dos usuários excluídos. Para obter mais informações, consulte [***Transcrições do aluno***](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md).
...
* **Digitalizar código QR para registrar:** os recursos de código QR foram aprimorados. Além do suporte à marcação de presença pelo código QR, esse recurso agora também oferece suporte à inscrição em cursos e à marcação da conclusão de um curso com o código QR.
  ...
  Para se inscrever e concluir um curso, você pode digitalizar um código QR fornecido pelo administrador. <!-- For more information on scanning QR codes in the web version of Learning Manager, see  [***Scan QR code***](https://helpx.adobe.com/captivate-Learning>Manager/whats-new.html#QRcodetoenrollcompleteenrollcompleteacourse) -->.

* **Várias tentativas no curso:** o aplicativo Learning Manager permite que o aluno realize cursos com várias tentativas habilitadas. <!-- For more information on setting up multiple attempts, see  [***Multiple attempts***](<https://helpx.adobe.com/captivate-Learning>Manager/authors/feature-summary/courses.html#Multiattempts). -->
...
* A inscrição de usuários podia ocorrer de forma incorreta quando eles eram removidos de um grupo de usuários. Esse problema foi corrigido.
...
* A certificação podia estar ausente no relatório suspenso do painel, embora estivesse visível no aplicativo do gerente e na lista de API de dados. Esse problema foi corrigido.
* Alguns objetos de aprendizado não podiam ser desativados pelo administrador devido à falta de permissões, embora os catálogos compartilhados fossem independente das contas do Learning Manager. Esse problema foi corrigido.
...
* A inserção de notificações do usuário pode falhar aleatoriamente e, como consequência, os e-mails associados não são entregues. Esse problema foi corrigido.
...
* As informações sobre atividades como inscrição/conclusão podem não estar presentes na transcrição do aluno baixada pelo gerente. Isso foi corrigido.
...
* Esta versão do Adobe Learning Manager traz novos recursos e aprimoramentos. Ela oferece a capacidade de criar funções personalizadas, adicionar rótulos de catálogo, limpar usuários, gerenciar marcas, renomear objetos de aprendizado, integração do Slack, novas integrações do conector, suporte a xAPI e muito mais. Para obter mais informações sobre os novos recursos e aprimoramentos, consulte [Resumo dos novos recursos](../whats-new.md#main-pars_text).
...
* O hiperlink para o número dos cursos e certificações dentro da janela modal de etiquetas inclui cursos fantasmas e certificações repetidas. Ao clicar no hiperlink, estes cursos e certificações podem não ser listados, causando discrepância nos números.
...
* Quando você pesquisa e seleciona qualquer aluno e marca a presença do aluno, o Learning Manager pode marcar a presença para todos os alunos. Isso foi corrigido.
* O Learning Manager exibiria nos e-mails a hora no formato de 24 horas. Esse problema foi corrigido. A hora é exibida agora no formato de 12 horas.
...
* O professor não conseguia marcar a presença para todos os usuários de uma só vez nas sessões que faziam parte de programas de aprendizado/certificações. Isso foi corrigido.
...
**Solução:** Atualize o navegador Internet Explorer 11 para a versão mais recente entrando em contato com a equipe de TI da sua empresa.
...
* Quando o catálogo padrão é desativado e um novo catálogo é criado, e se o gerente tem acesso a este catálogo recém-criado, ele talvez não consiga pesquisar nenhum curso em tal catálogo. Esse problema foi corrigido.
* Os usuários do aplicativo móvel podem não receber feedback N1 como notificações push. Esse problema foi corrigido.
...
* As alterações feitas em um curso compartilhado podiam não ser propagadas para a conta compartilhada se o autor salvasse automaticamente o curso na conta principal. Esse problema foi corrigido.
...
* Certos e-mails do Learning Manager, como e-mails sem dados de notificação, não estavam sendo acionados para os usuários. Por exemplo, o e-mail recebido ao criar e ativar o perfil externo e o e-mail recebido ao configurar uma conta do Connect. Esse problema foi corrigido.
...
* Em alguns casos, após adicionar um programa de aprendizado a um catálogo, a opção para adicionar ainda podia ser ativada. Esse problema foi corrigido.
...
O cronograma do quadro de classificação da gamificação do aluno interno podia mostrar aluno externo se ele fosse convertido de aluno externo a aluno interno. Esse problema foi corrigido.
...
* Ao clicar no índice, ele podia não se contrair automaticamente e precisava ser fechado manualmente para visualizar o conteúdo. Esse problema foi corrigido.
...
* O índice não se fechava automaticamente quando a orientação do dispositivo era alterada. Esse problema foi corrigido.
...
* O botão Fechar do reprodutor não era visível em determinados projetos quando a orientação do dispositivo era alterada. Esse problema foi corrigido.
...
* Se um usuário não tivesse baixado nenhuma ajuda de tarefa, ao clicar na guia Minhas ajudas de tarefa no modo off-line o aplicativo podia travar no iOS e mostrar uma mensagem de erro de carregamento de dados no Android. Isso foi resolvido.
...
* Uma mensagem de erro podia ser exibida em determinadas ocasiões ao tentar remover uma ajuda de tarefa que já tinha sido adicionada a partir da guia Minhas ajudas de tarefa. Esse problema foi corrigido.
...
Esta versão aumenta o escopo da gamificação. Os usuários externos agora podem participar da gamificação. Como administrador, você pode definir o escopo da gamificação alterando as configurações do escopo. É possível ativar de forma seletiva a gamificação entre usuários, grupos ou locais de perfil similar.
...
As ajudas de tarefa são um conteúdo de treinamento a que os alunos podem ter acesso sem ter de se inscrever em um objeto de aprendizado específico, como um curso ou programa de aprendizado. Com esse aprimoramento, os administradores podem extrair e fazer download de relatórios das ajudas de tarefa. Como administrador, você também pode gerar um relatório de todos os comunicados enviados por você. Os administradores e os gerentes também podem extrair um relatório dos alunos que cancelaram a inscrição.

**Conectores do Learning Manager**

Agora é possível exportar habilidades do usuário para um local FTP para fazer a integração com qualquer sistema de terceiros usando a opção de exportação dos dados. Especifique o nome da conexão da integração e escolha se deseja importar usuários internos ou exportar habilidades do usuário configurando-os ou buscando-os quando necessário.

**Copiar instâncias do curso**

Você já pode copiar o URL da instância clicando na seta suspensa no canto superior direito de uma instância.
...
Os lembretes de prazo de conclusão comunicavam o número de dias restantes incorretamente para alunos em determinados fusos horários. Esse problema foi corrigido.
...
Use os widgets da página inicial para controlar seus cursos, habilidades e realizações. Use a barra de pesquisa para realizar pesquisas em todo o LMS que inclui todos os objetos de aprendizado, catálogos, habilidades, notas e discussões
...
Ao criar objetos de aprendizado, agora os autores e os administradores podem especificar IDs exclusivas para cursos, programas de aprendizado ou certificações. Se desejar ativar a ID exclusiva ao criar um objeto de aprendizado, clique em Configurações > Geral. Selecione a caixa de seleção Ativar ao lado da opção IDs exclusivas do objeto de aprendizado.
...
* Na função do aluno, a eficácia do curso era exibida como uma das opções de filtro **Classificar por** mesmo quando um administrador desativava a eficácia do curso dos alunos. Esse problema foi corrigido.
...
* Os cursos de rascunho também eram retornados em resposta quando o administrador tentava obter todos os cursos.Estes cursos preliminares são supostamente privados para o autor. Esse problema foi corrigido e os cursos de rascunho agora não são devolvidos.
...
Também aprimoramos o modelo de usuários existente com as seguintes adições:
...
O Learning Manager fornece um recurso para gerar transcrições para os alunos do Learning Manager da sua organização. Para obter mais informações, consulte o [conteúdo de ajuda do recurso Transcrições do aluno](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md).
...
* Depois de adicionar um grupo de usuários externo ao Learning Manager, você também pode pausar o processo de registro de usuários externos. A qualquer momento, você pode revogar o bloqueio (pausa) selecionando a opção Continuar.
...
* Você pode assinar relatórios de cursos, programas de aprendizado ou certificados. Você também pode assinar relatórios de curso individuais para obter dados como o status da pontuação do questionário e do aluno. As assinaturas serão enviadas à sua id de e-mail conforme registrada na conta do Learning Manager. Você também pode alterar esta id de e-mail.
* Quando você exporta o relatório de inscrição de certificação, uma nova coluna denominada **Data de vencimento** também é exportada. Os dados dessa coluna permitem que os administradores saibam quais alunos ultrapassaram o prazo de realização do objeto de aprendizado.
...
* Quando um curso do Presenter com um título que continha apóstrofo (&#39;) é publicado e exibido no Learning Manager, pontos de interrogação (???) usado para aparecer no sumário. Esse problema foi corrigido.
...
* Ao criar certificações, se um administrador inserir o valor de **dias para concluir**, que é maior ou igual ao período de validade da certificação, uma mensagem de aviso será exibida. Anteriormente, a mensagem de aviso não era mostrada para os administradores.
...
Você pode renovar as certificações em termos de meses indicando o valor no campo **Vigência**. Anteriormente, a renovação da certificação era permitida somente em anos.
...
* Quando um aluno externo tentava abrir os módulos de avaliação e trabalho prévio usando o link **Abrir notas** na notificação de Rever curso enviada por e-mail, o conteúdo das notas não estava visível. Esse problema foi corrigido.
...
Os nomes de título em modelos de e-mail não eram apropriados. O conteúdo foi atualizado nesses títulos do modelo para torná-los legíveis.
...
As ajudas de tarefa correspondem a um repositório de conteúdo de treinamento acessível aos alunos sem nenhum tipo de inscrição nem critérios de conclusão. Os alunos podem consultar essas ajudas de tarefa para obter assistência na execução de qualquer atividade ou tarefa em uma empresa. O administrador pode controlar o número de downloads por ajuda de tarefa.
...
Os eventos dos planos de aprendizado foram atualizados. Foram adicionados um par de novos eventos. Consulte [Planos de aprendizado](../learners/feature-summary/learning-programs.md) para obter mais informações.
...
Os administradores podem definir o escopo da gamificação alterando as configurações do escopo. É possível ativar de forma seletiva a gamificação entre usuários, grupos ou locais de perfil similar. Consulte o recurso [Gamificação](../learners/feature-summary/gamification.md) para obter mais informações.
...
Agora você pode excluir cursos nas fases de rascunho e retirado. Consulte o recurso [Cursos](../administrators/feature-summary/courses.md) para mais informações. Se um objeto de aprendizado for excluído, todos os seus dados de relatório também serão excluídos. Se um curso é excluído e ele fizer parte de qualquer outro objeto de aprendizado, uma mensagem apropriada é exibida ao usuário.

**Planos e programas de aprendizado**

Você pode impor a ordem em que os alunos podem realizar os cursos dentro dos programas de aprendizado. Você pode excluir programas de aprendizado nas fases de rascunho e retirado. Se um objeto de aprendizado for excluído, todos os seus dados de relatório também serão excluídos.

**Certificações**

Você pode excluir certificações nas fases de rascunho e retirado. Se um objeto de aprendizado for excluído, todos os seus dados de relatório também serão excluídos.
...
Quando os usuários clicam em **[!UICONTROL Fazer logoff]** no Learning Manager, agora eles saem do aplicativo Learning Manager e também se desconectam das suas IDs da Adobe.
...
* Na função Administrador, o nome completo dos alunos é indicado na guia **Alunos**. Anteriormente, somente o primeiro nome dos alunos era exibido.
...
* Ao acessar cursos, catálogos e programas de aprendizado nas funções Administrador e Autor, a ordem das guias é alterado para **Publicado - Rascunho - Tudo - Retirado**. A seleção padrão é **Publicada.**
...
* Na função Administrador, quando um usuário introduz o número de alunos e clica em **Fazer pedido**, é exibida uma caixa de diálogo com detalhes sobre cobranças mensais e anuais da assinatura por usuário.
