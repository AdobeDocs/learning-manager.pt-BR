...
Em geral, as grandes empresas têm seu LMS interno ou qualquer sistema de gerenciamento de aprendizagem legado fornecido pelo fornecedor. O LMS é composto de conteúdo de treinamento e dados de treinamento da empresa. Assim como uma empresa, quando você compra o Learning Manager, talvez você queira mover o conteúdo e os dados existentes do LMS para o Learning Manager, porque dessa forma você pode aproveitar os benefícios do moderno e intuitivo LMS sem perder alguns dados legados da sua empresa.
...
### Aviso importante sobre migração

Você deve estar ciente de que os cronogramas de migração dependem muito da qualidade e do tamanho dos dados. Se precisar migrar durante a integração, planeje essa atividade com bastante antecedência e trabalhe em conjunto com a equipe de integração do Adobe Learning Manager para evitar atrasos.
...
1. O administrador de integração usa as ferramentas fornecidas pelo Learning Manager para migrar na seguinte ordem:
   ...
   1. Transfira o conteúdo de treinamento para o Learning Manager e
...
Antes de dar início ao procedimento de migração, é importante observar os seguintes pontos:
...
A criação de contas FTP ou Box é um pré-requisito para criar projetos de migração. Portanto, nesta etapa, você pode criar um projeto e um sprint de migração no aplicativo Learning Manager.  Consulte a seção **Procedimento de migração de dados e conteúdo** nesta página para criar projetos de migração.
...
## Migração para Alternativas e Equivalentes

### Visão geral

Este tópico descreve o modelo de dados baseado em CSV e o comportamento de migração para introduzir a equivalência do objeto de aprendizado (LO) no sistema.

### Arquivos CSV existentes (contexto)

Esses CSVs já existem na plataforma e fornecem o objeto de aprendizado principal, o módulo e o contexto de conclusão (lista não completa):
...
Esses arquivos continuam a ser usados como estão e não são alterados pelo novo recurso de equivalência, mas formam os dados subjacentes sobre os quais a equivalência funcionará.

### Novos arquivos CSV para alternativas

Dois novos CSVs foram introduzidos para oferecer suporte a relacionamentos alternativos de LO e conclusões de usuário relacionadas.

#### &#x200B;1. equivalence_relationship.csv

Define mapeamentos de equivalência entre os objetos de aprendizado (LOs) de origem e de destino, que podem ser cursos ou caminhos de aprendizado (LPs).

**Esquema:**

* sourceId
* sourceloType (curso/LP)
* targetId
* targetLotype (curso/LP)
* dateCreated
* relationshipStatus (ATIVO / DELETE)
* dateModified

**Finalidade:**

* Representa uma relação de equivalência entre dois OAs.
* relationshipStatus controla se a relação está ativa ou excluída no momento.
* auditoria de suporte dateCreated e dateModified.

#### equivalence_user_completion.csv

Captura informações de conclusão no nível do usuário para OAs equivalentes, alinhadas com as relações definidas em equivalence_relationship.csv.
...
* Registra explicitamente quais **conclusões do OA de destino** devem ser deduzidas para um usuário com base na relação de equivalência e na conclusão do OA de origem existente.
* Funciona como a **fonte autoritativa** para conclusões de usuário ligadas a dados equivalentes migrados.

### Regras de migração e semântica comportamental

#### &#x200B;1. Não há suporte para aprimoramento de novos CSVs equivalentes

* Todos os dados relacionados à equivalência devem ser trazidos via migração.
* O sistema não oferecerá suporte a cenários em que:
  * Os dados do OA (cursos/LPs) foram criados por meio da interface do usuário e
  * As relações de equivalência são importadas posteriormente somente por meio de CSV.

Isto significa:

* O padrão suportado é: As definições do LO e suas relações de equivalência são gerenciadas como parte de um fluxo de migração coerente.
* Os fluxos híbridos em que os OA criados pela interface do usuário são adaptados com equivalência somente por CSV não são compatíveis.

#### &#x200B;2. Nenhuma conclusão retroativa/inconclusão de relacionamentos migrados

Quando uma relação de equivalência é introduzida por meio da migração (ou seja, por meio de equivalence_relationship.csv):

* O sistema não executará cálculos retroativos de conclusão ou inconclusão baseados apenas nesse relacionamento.
* Em vez disso, todos os dados de conclusão do usuário necessários devem ser fornecidos explicitamente por meio do equivalence_user_completion.csv.

**Implicação:**

* equivalence_user_completion.csv é a única fonte confiável para quaisquer conclusões que devem ser reconhecidas no momento da migração como resultado da equivalência.
* A plataforma não tentará inferir ou reverter essas conclusões do progresso do curso existente.

#### &#x200B;3. Comportamento de novas conclusões após a migração

Se:

* Uma relação de equivalência foi criada por meio da migração e
* Um aluno conclui posteriormente o LO de origem (pós-migração),

depois:

* O sistema acionará conclusões alternativas para o OA de destino, ou seja, a equivalência se comporta normalmente daqui para frente para novas conclusões de origem.

**Distinção de chave:**

* **No momento da migração:** as conclusões devem vir por meio de equivalence_user_completion.csv.
* **Após a migração:** a lógica de tempo de execução nativa tratará de conclusões alternativas quando um LO de origem for concluído recentemente.

#### &#x200B;4. Impacto em objetos de aprendizado de ordem superior

Conclusões alternativas recebidas por meio de CSV (por exemplo, via equivalence_user_completion.csv) acionarão o recálculo de LOs de ordem superior.

Os OAs de ordem superior podem incluir:

* Caminhos de aprendizado

**Implicação técnica:**

* A assimilação de equivalence_user_completion.csv não é uma operação “silenciosa”: ela inicia a mesma lógica de recálculo/roll-up que seria acionada por términos normais de tempo de execução.
* Os sistemas que integram ou agendam essa migração devem planejar a carga e o tempo de recálculos.

## Webhooks para alternativas

Quando um aluno conclui um curso por meio de uma inscrição alternativa ou por meio de um relacionamento, o Adobe Learning Manager gera um evento de webhook dedicado diferente do webhook de conclusão de curso padrão, permitindo que as integrações apliquem uma lógica de tratamento diferente para conclusões alternativas. Os eventos do webhook também são gerados para conclusão retroativa e inconclusão retroativa, abrangendo alterações históricas no status do curso, incluindo aquelas orientadas por atualizações de relacionamentos, para que os sistemas externos permaneçam sincronizados com o estado de conclusão atual do aluno.

Para obter informações sobre webhooks para Alternativas, exiba [Webhooks para Alternativas](/help/migrated/integration-admin/feature-summary/webhooks.md#webhooks-for-alternates)

## Procedimento de migração de dados e conteúdo {#dataandcontentmigrationprocedure}

O procedimento para migrar dados e conteúdo do LMS corporativo para o Learning Manager é descrito como se segue:
...
O Learning Manager oferece suporte à UUID para proporcionar flexibilidade às empresas no controle de contas de usuários. Como administrador, se tiver a UUID dos usuários em uma conta, você pode modificar as IDs de e-mail dos usuários de tal conta.
...
Para obter mais informações sobre esse tópico, consulte o seguinte conteúdo da Ajuda:
...
A versão de abril de 2026 do Adobe Learning Manager oferece aprimoramentos direcionados à API pública nas áreas de alternativas e equivalentes, acesso ao conteúdo em janela de tempo, tentativas de questionário orientadas por conteúdo, experiências de aluno não conectado e gerenciamento de ajuda de tarefa. Essas atualizações foram projetadas para permanecerem amplamente compatíveis com versões anteriores, permitindo padrões de integração mais precisos e extensíveis.

Para alterações de API, exiba [alterações de API](/help/migrated/api-changes-alm.md).

## Migração da sessão do VILT para o Adobe Learning Manager {#migrationofviltsessiontoalm}

O Adobe Learning Manager oferece suporte à migração em massa e à atualização de dados de sessão do VILT (Virtual Instructor-Led Training, treinamento virtual ministrado por instrutor) por meio de arquivos CSV. Use esse fluxo de trabalho para configurar datas de início da instância, associar instâncias do caminho de aprendizado a instâncias do curso e configurar sessões de sala de aula virtual para Microsoft Teams, Adobe Connect e Zoom.

>[!NOTE]
>
>As IDs de coluna em todos os arquivos CSV de migração agora usam o prefixo alm, por exemplo, `almCourseID` e `almModuleID`. Isso substitui o prefixo principal herdado usado em versões anteriores.

### Migração de sessão VILT baseada em CSV

A migração do Adobe Learning Manager permite que os administradores criem ou atualizem em massa o conteúdo de aprendizado usando arquivos CSV estruturados. Você pode aplicar esses fluxos de trabalho CSV a cursos de migração (conteúdo importado de um sistema externo) e cursos de aprimoramento (conteúdo criado diretamente no aplicativo do autor do ALM).

Quatro arquivos CSV estão envolvidos na migração da sessão VILT:

* **CSV da instância do curso:** cria ou atualiza instâncias do curso, incluindo datas de início
* O **CSV da Instância LP:** cria ou atualiza instâncias do caminho de aprendizado, incluindo datas de início
* **LP para CSV de associação de instância de curso:** mapeia uma instância do caminho de aprendizado para uma instância específica do curso
* **O CSV de sessão:** cria sessões de sala de aula virtual com detalhes do sistema de conferência

Baixe os arquivos acima [aqui](assets/csv-and-xlsx-migration-files.zip).

Todos os quatro arquivos CSV aceitam `almCourseID` para fazer referência a cursos e `almModuleID` para fazer referência a módulos. Essas IDs são os identificadores exclusivos atribuídos pelo ALM quando um curso ou módulo é criado.

### Definir a data de início das instâncias do curso e do caminho de aprendizado

Use o **CSV da Instância do Curso** e o **CSV da Instância LP** para adicionar ou atualizar a data de início em uma instância. Isso se aplica a instâncias criadas por migração e por interface (retrofit).

**CSV da instância do curso: adicionar uma data de início**

1. Abra o arquivo CSV da instância do curso.
2. Adicione a coluna `startDate` se ela ainda não estiver presente.
3. Insira a data de início para cada linha de instância no formato AAAA-MM-DD.
4. Preencha a coluna `almCourseID` com a ID do curso do ALM para o curso que deseja atualizar.
5. Faça upload do CSV durante a execução da migração.

**CSV de Instância LP: adicione uma data de início**

1. Abra o arquivo CSV da instância LP.
2. Adicione a coluna `startDate` se ela ainda não estiver presente.
3. Insira a data de início para cada linha de instância no formato AAAA-MM-DD.
4. Popular a coluna `almLearningProgramID` com a ID do caminho de aprendizado do ALM.
5. Faça upload do CSV durante a execução da migração no.

>[!NOTE]
>
>A coluna `startDate` é opcional. Se você incluí-lo, o valor deve ser anterior a `completionDate`. As linhas em que `startDate` for posterior a `completionDate` apresentarão erros e aparecerão na migração.

### Associar instâncias do caminho de aprendizado a instâncias do curso

Use o LP para o CSV da associação de instância de curso para vincular uma instância do caminho de aprendizado a uma instância específica do curso. Essa etapa é necessária para cursos VILT que fazem parte de um caminho de aprendizado.

1. Abra o LP no arquivo CSV da Course Instance Association.
2. Para cada linha, preencha as seguintes colunas:
a. `almLearningProgramID` — a ID do caminho de aprendizado do ALM
b. `almLearningProgramInstanceID` — a ID da instância do caminho de aprendizado do ALM
c. `almCourseID` — a ID do curso ALM
d. `almCourseInstanceID` — a ID da instância do curso ALM
3. Faça upload do CSV durante a execução da migração.

### Cenários de associação com suporte

Nem todas as combinações de origens de migração e retrofit são suportadas. Revise a tabela abaixo antes de criar seu CSV.

| Origem do caminho de aprendizado | Origem da instância do curso | Compatível |
|-----------------------------|-------------------------------|-----------|
| Migração | Migração | Sim |
| Retrofit (criado por interface do usuário) | Retrofit (criado por interface do usuário) | Sim |
| Migração | Retrofit (criado por interface do usuário) | Não |
| Retrofit (criado por interface do usuário) | Migração | Não |

>[!NOTE]
>
>Se precisar associar uma instância do caminho de aprendizado de aprimoramento a uma instância do curso de migração (ou vice-versa), adicione o curso ao caminho de aprendizado diretamente pelo aplicativo do autor do ALM em vez de usar este CSV.

### Configurar detalhes da sessão da sala de aula virtual

Use o **CSV de sessão** para criar ou atualizar sessões VILT com detalhes de conferência de sala de aula virtual. Quatro colunas foram adicionadas ao CSV da sessão para oferecer suporte a isso:

| Coluna | Descrição |
|--------------|-------------------------------------------------------|
| `almCourseID ` | ID do ALM do curso |
| `almModuleID` | ID do ALM do módulo |
| `metadata` | Objeto JSON contendo configuração específica do sistema VC |
| `meetingID` | ID de reunião do sistema de aula virtual externo |

### Formato de metadados por sistema de conferência

O campo `metadata` aceita um objeto JSON. A estrutura varia de acordo com o sistema de conferência. Todos os nomes de chave fazem distinção entre maiúsculas e minúsculas e devem usar camelCase **exatamente como mostrado.**
...
Todos os campos de metadados do Teams são opcionais. Se você não fornecer `organizerEmail`, o ALM usará o email do administrador de equipes configurado na sua conta do ALM como o organizador padrão.
...
O campo `primaryInstructor` é **obrigatório** para sessões do Adobe Connect. Todos os outros campos são opcionais. Você pode fornecer `persistentRoom` ou `templateID`. Se você fornecer `templateID`, o ALM criará a sala usando esse modelo.

**Zoom**

O zoom não requer um objeto JSON de metadados. Transmita o professor da sessão usando a coluna do professor padrão no CSV da sessão.

### Fazer upload do CSV da sessão

1. Abra o arquivo CSV da sessão.
2. Adicione as quatro novas colunas: almCourseID, almModuleID, metadados e meetingID.
3. Para cada linha de sessão, preencha almCourseID e almModuleID com as IDs do ALM do curso e do módulo.
4. Adicione a meetingID do seu sistema de sala de aula virtual (Teams, Adobe Connect ou Zoom).
5. Crie o objeto JSON de metadados usando o formato do seu sistema de conferência.
6. Certifique-se de que todos os nomes de chave JSON usem a grafia exata de camelCase. O uso incorreto de maiúsculas e minúsculas causa falha na linha.
7. Faça upload do CSV durante a execução da migração.

Solução de problemas comuns de migração

| Problema | Solução |
|-------|----------|
| Erros de linha com “O prazo de conclusão deve ser maior que a data de início” | Verifique se `startDate` é anterior a `completionDate` no CSV da instância. |
| Falha na associação da LP à instância do curso | Confirme se o caminho de aprendizado e a instância do curso foram criados pela mesma origem (migração ou aprimoramento). Fontes mistas não são suportadas. |
| A linha da sessão falha com erro de metadados | Verifique se todos os nomes de chave JSON no campo `metadata` usam camelCase exato. As chaves diferenciam maiúsculas de minúsculas. |
| Equipes `isCompletionCriteria` não têm efeito | O sinalizador de recurso de critérios de conclusão para equipes deve ser habilitado pelo administrador da conta do ALM antes que os valores da migração entrem em vigor. |
| Linha de sessão criada, mas o campo de professor está vazio | Se o email do professor fornecido não corresponder a um usuário no ALM, a sessão é criada com um campo de professor vazio. Verifique se o email do professor existe no ALM antes de fazer upload. |

## Migrar módulos de LTI {#migrationofltimodules}

### Visão geral

A migração de LTI amplia o fluxo de trabalho de migração existente e não exige arquivos de migração adicionais. Os registros de associação de curso, módulo e módulo existentes continuam a usar o formato de migração padrão. As informações específicas de LTI são fornecidas por meio dos dados de versão do módulo.

### Usar arquivos para migração de LTI

Os módulos de LTI são migrados usando os arquivos de migração padrão.

Os seguintes arquivos continuam a usar o formato de migração existente:

* course.csv
* module.csv
* course_module.csv

Nenhum campo específico de LTI é necessário nesses arquivos. Configurações específicas de LTI estão definidas no arquivo `module_version.csv`.

### Configurar uma versão do módulo de LTI

Use o arquivo `module_version.csv` para definir as propriedades de uma versão de módulo de LTI.

Além dos campos existentes com suporte no `module_version.csv`, o Adobe Learning Manager oferece suporte a valores e atributos específicos de LTI.

#### contentType

Use o valor `LTI` no campo `contentType` para identificar a versão do módulo como um módulo de LTI.
...
Especifica a URL de inicialização do provedor de LTI externo.

Quando um aluno inicia o módulo no Adobe Learning Manager, ele é redirecionado para o ponto de extremidade de LTI configurado.

*Campo usado para especificar a URL de inicialização do provedor de LTI externo*

| **Campo** | **Descrição** |
|--------------|--------------------------------------------------|
| ltiLaunchUrl | URL de inicialização fornecida pela plataforma LTI externa |

#### ltiCustomParams

Especifica parâmetros de inicialização personalizados que são passados para o provedor de LTI durante a inicialização.

Use este campo quando a plataforma externa exigir parâmetros de configuração ou contexto de inicialização adicionais.

*Campo usado para passar parâmetros de inicialização personalizados para o provedor de LTI*

| **Campo** | **Descrição** |
|-----------------|------------------------------------------------------------|
| ltiCustomParams | Parâmetros personalizados passados para a plataforma de LTI durante a inicialização |

#### tpName

Especifica o nome do provedor de LTI de terceiros associado ao módulo.

*Campo usado para identificar o provedor de LTI de terceiros*

| **Campo** | **Descrição** |
|-----------|-----------------------------------------------------------------|
| tpName | Nome do provedor de LTI de terceiros associado ao módulo |

### Amostra de versão do módulo de LTI

O exemplo a seguir mostra um registro de versão do módulo configurado para um módulo de LTI:
...
* A versão do módulo é identificada como um módulo de LTI por meio do valor `contentType=LTI`.
* O URL de inicialização aponta para o provedor de LTI externo.
* Os parâmetros de inicialização personalizados são fornecidos por meio de `ltiCustomParams`.
* O provedor é identificado pelo campo `tpName`.

### Migrar um módulo de LTI

Para migrar um módulo de LTI:

1. Crie o registro do curso em `course.csv`.
2. Crie o registro do módulo em `module.csv`.
3. Associe o curso e o módulo em `course_module.csv`.
4. Adicione os detalhes da versão do módulo em `module_version.csv`.
5. Defina o valor `contentType` como `LTI`.
6. Forneça o URL de inicialização da LTI e os parâmetros de inicialização opcionais.
7. Execute o sprint de migração.

A estrutura de migração processa o módulo de LTI como parte do fluxo de trabalho de migração padrão.

### Validar versões do módulo de LTI

Ao criar versões do módulo de LTI:

* Use o valor `LTI` para o campo `contentType`.
* Forneça uma URL de inicialização válida no campo `ltiLaunchUrl`.
* Especifique o nome do provedor externo no campo `tpName`.
* Certifique-se de que o módulo esteja associado a um curso por meio dos arquivos de migração padrão.
