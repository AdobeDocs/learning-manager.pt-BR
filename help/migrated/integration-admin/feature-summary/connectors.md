...
As empresas usam outros aplicativos e sistemas que precisam ser integrados com o Learning Manager. Os conectores são utilitários que ajudam na execução de integrações baseadas em dados, como a importação de dados no Gerenciador de aprendizagem de sistemas externos.  Também realiza a exportação de dados para sistemas externos pelo Learning Manager.
...
Saiba mais sobre o processo para integrar o Salesforce com o Learning Manager

#### Pré-requisitos {#prerequisites}

Certifique-se de que sua organização do Salesforce compartilhe um URL com você. Por exemplo, se o nome da sua organização é **myorg**, o URL do Salesforce pode ser `https://myorg.salesforce.com`. Esta é a única entrada necessária para conectar a conta do Salesforce ao Learning Manager.
...
1. É exibida uma caixa de diálogo solicitando que você insira o URL da organização. Clique **[!UICONTROL em Conectar]** depois de fornecer o URL.
1. Após uma conexão bem-sucedida, a página de visão geral é exibida.

### Mapear atributos {#mapattributes}

Uma vez que a conexão é estabelecida com sucesso, você pode mapear as colunas do Salesforce para os atributos correspondentes do Gerenciador de aprendizado. Essa etapa é obrigatória.

1. No lado esquerdo da página de mapeamento você pode ver as colunas do Gerente de aprendizado e, no lado direito, as colunas do Salesforce. Selecione o nome da coluna apropriada que faz mapeamento para o nome de coluna do Gerenciador de aprendizado.
   ...
   *Mapear atributos*
   ...
   >[!NOTE]
   >
   >Os dados da coluna do Gerente de aprendizado mostrados no lado esquerdo são coletados dos campos ativos. O **campo Gerente** deve ser mapeado para um campo do tipo endereço de email. Antes de usar o conector, é necessário mapear todas as colunas.

1. Clique **[!UICONTROL em Salvar]** depois de concluir o mapeamento.
...
Na página do conector do Salesforce, insira o URL do Salesforce e conclua a autenticação. Depois de autenticar, você pode importar usuários ou contatos. Se você escolher a opção Contatos, especifique o subconjunto de contatos a ser importado.

Escolha as colunas do Salesforce e mapeie-as para os atributos agrupáveis correspondentes do Gerenciador de aprendizagem. Uma vez que o mapeamento é concluído, este mesmo mapeamento é usado em importações subsequentes do usuário.

1. Faça logon no Salesforce.
1. Na página de conexão, clique **[!UICONTROL em Importar usuários]** internos.

   ![](assets/image048.png)
   *Importar usuários internos*

1. **Na página Importar usuários**, há uma nova opção, Contatos. Clique no botão **de opção Contatos** e verá as seguintes opções.

   ![](assets/image050.png)
   *Mapear os atributos de contato*

1. Se clicar **[!UICONTROL em Sim]**, você pode realizar o seguinte:

   * **Coluna Contatos:** selecione o campo que deseja importar para o Gerenciador de aprendizagem.
   * **Especificar valores:** escolha os valores que representam o campo selecionado.

   ![](assets/image053.png)
   *Especifique os valores*

   * Mapeie as colunas do Salesforce com a do Gerenciador de aprendizado.
   * Para começar a importar, clique **[!UICONTROL em Salvar]**.

1. Se você clicar em **[!UICONTROL Não. Importar todos os contatos]**, é possível mapear os campos diretamente sem filtrar os contatos. Aqui, você importaria todos os contatos do Salesforce.
1. Para começar a importar, clique **[!UICONTROL em Salvar]**.

## Exportar registros de aprendizado

O Gerenciador de aprendizagem oferece a capacidade de exportar registros de aprendizado como transcrição, relatório do usuário, relatório de habilidade para Salesforce. Você pode determinar se os dados exportados devem ser vinculados à tabela &quot;Usuário&quot; ou à tabela &quot;Contatos&quot; no Salesforce.
...
Antes de exportar registros de aprendizado do Gerenciador de aprendizagem, você deve criar objetos personalizados no Salesforce. Objetos personalizados são objetos que você cria para armazenar informações específicas da sua empresa ou setor. Para obter mais informações, consulte [Objetos personalizados do Salesforce](https://trailhead.salesforce.com/en/content/learn/modules/data_modeling/objects_intro).
...
>[!NOTE]
>
>Certifique-se de que o acesso do administrador de sistema tenha sido concedido a todos os campos ativos adicionados após a instalação do pacote.

**Vincular eventos com:** Escolha a seção que deseja exportar- Usuário ou Contato. Se você escolher o objeto Contato, os usuários que estão presentes no Gerenciador de aprendizado, mas não no Salesforce, serão criados no Salesforce.

![](assets/link-events.png)
*Opção de eventos de link*
...
>[!NOTE]
>
>Na página Status de execução do Salesforce, o número de registros processados só pode ser verificado no Salesforce. O Gerente de aprendizado exibe o status como concluído mesmo quando há uma exportação parcial ou falha em todos os registros que foram processados.
...
1. Inicie o URL do pacote do [Gerenciador de aprendizado](https://login.salesforce.com/packaging/installPackage.apexp?p0=04t1k0000008WOQ).
...
1. Clique **[!UICONTROL em Instalar]**. Quando o pacote estiver instalado, clique **[!UICONTROL em Concluído]**. Você é guiado até a página Pacotes instalados e poderá ver o pacote instalado do Adobe Learning Manager.
...
1. Em **[!UICONTROL Pesquisa]** rápida, pesquise por Configurações de site remoto.
...
### Ativar as notificações para o aplicativo do Gerenciador de aprendizagem

1. No canto superior direito, clique **[!UICONTROL em Configurar]**.
...
Clique **[!UICONTROL em Configurar]** > **[!UICONTROL novo]** e adicione o seguinte:
...
*Definir permissões para acessar o aplicativo do Gerenciador de aprendizagem*
...
Se a opção Caminho do aprendizado estiver desativada na conta de Administrador, nenhuma linha e coluna são adicionadas ao relatório.

Se a opção Caminho do aprendizado estiver habilitada na conta de Administrador, a coluna &quot;Tipo&quot; será preenchida com o caminho do aprendizado caso os alunos estejam inscritos nele.

>[!NOTE]
>
>Se o sinalizador estiver ativado e você usar uma conexão existente, alguns registros poderão ser perdidos.

#### Novas conexões

Se a opção Caminho do aprendizado estiver desativada na conta de Administrador, o relatório de treinamento será composto pelas seguintes colunas, mas não conterá dados.
...
* **ID incorporada do curso:** exibe as IDs dos cursos que estão dentro de um caminho de aprendizado.
...
Além disso, o relatório conterá o tipo de coluna Caminho de aprendizagem (nível superior) para todos os alunos que estiverem inscritos em um caminho de aprendizagem.

Na coluna Tipo, o programa de aprendizado será renomeado como Caminho do aprendizado. Para conexões existentes, não haverá alterações.
...
Também é possível usar o conector do Box para migrar dados, importar o usuário e exportar dados. Para obter mais informações, consulte o conector do Box.
...
1. É exibida uma caixa de diálogo solicitando que você insira a ID de e-mail. Forneça a ID de e-mail da pessoa responsável por gerenciar a conta FTP do Gerenciador de aprendizagem da organização. Clique **[!UICONTROL em Conectar]** após fornecer a id de e-mail.
1. O Learning Manager envia um e-mail solicitando que o usuário redefina a senha antes de acessar o FTP pela primeira vez. O usuário deve redefinir a senha e usá-la para acessar a conta FTP do Learning Manager.
   ...
   Na página de visão geral, você pode especificar o nome de conexão da sua integração. Escolha que ação você deseja executar entre as seguintes opções:
   ...
   * Exportar transcrições do aluno - Sob comando
...
A opção importar usuário interno permite que você importe os usuários de um csv para um Gerente de aprendizado sob demanda ou agendamento.
...
1. No lado esquerdo da página Mapear atributos você pode ver as colunas esperadas do Gerente de aprendizado e, no lado direito, os nomes das colunaS CSV. Inicialmente, no lado direito, você verá uma caixa de seleção em branco. Importe qualquer modelo CSV clicando **em Escolher arquivo**.
1. A etapa acima preenche a lista suspensa de seleção do lado direito com todos os nomes de colunas CSV. Selecione o nome da coluna apropriada que faz mapeamento para o nome de coluna do Gerenciador de aprendizado.
...
>[!NOTE]
>
>Ao importar usuários para o Learning Manager, o administrador também deve saber como os usuários são gerenciados no Learning Manager. Consulte a [Ajuda](migration-manual.md#usermanagement) do gerenciamento de usuários para obter mais informações.
...
1. Na página de configuração, selecione uma configuração existente disponível na lista de configurações para importar instruções xAPI do CSV. Clique em editar ou **adicionar um novo link de configuração** para navegar até a página Configurar Importação-fontes.
...
1. No painel esquerdo, clique **[!UICONTROL em Execução]** por demanda.
...
Há duas opções para exportar relatórios de habilidade do usuário.
...
Os arquivos exportados automaticamente estão presentes no local **Início/Exportação/&#42;FTP_location&#42;**

Os arquivos exportados automaticamente são disponibilizados com o título skill_achievements_**dama data &#42;_até_&#42;&#42;.csv&#42;**
...
**** Configurar: esta opção permite que você agende a extração do relatório. Selecione a caixa de seleção Habilitar agendamento e especifique a data e hora de início. Também é possível especificar o intervalo no qual deseja que o relatório seja gerado e enviado.

![](assets/configure-report.png)

+++

Para abrir a pasta de exportação onde os arquivos exportados são colocados em seu FTP, abra o link para a pasta FTP fornecido na página de transcrição do aluno conforme mostrado abaixo

Os arquivos exportados automaticamente estão presentes no local **Início/Exportação/&#42;FTP_location&#42;**

Os arquivos exportados automaticamente são disponibilizados com o título learner_transcript_**dam de &#42;_data a_&#42;&#42;.csv&#42;**
...
1. No painel esquerdo, clique **[!UICONTROL em Configurar]**. Siga um destes procedimentos:
   ...
   *Configurar o conector do Lynda para Gerente de aprendizado*
...
1. Clique na conexão cuja configuração que você deseja exibir ou editar.
   ...
   *Configurar o conector do getAbstract para o Gerenciador de aprendizagem*
...
1. Também é possível clicar no Status da execução no painel esquerdo, a qualquer momento, para visualizar o resumo de todas as execuções deste conector, em ordem cronológica. Você pode ver a data de início e a duração da sincronização, o tipo da sincronização (se é ou não uma sincronização sob demanda) e o status da sincronização (se está em andamento ou concluída).
   ...
   [Planilha do Excel de amostra do feed do usuário do getAbstract](assets/report-export-20170401175342.xlsx)
...
1. Se já tiver uma conexão estabelecida, na página inicial, clique em Harvard ManageMentor > Gerenciar conexões. Clique na conexão cuja configuração existente você quer editar.
   ...
   *Configure o conector do HarvardManage Mentor para o Learning Manager*
...
O administrador de integração pode escolher colunas do Workday e mapeá-las para os atributos agrupáveis correspondentes do Gerenciador de aprendizagem. Uma vez que o mapeamento é concluído, este mesmo mapeamento é usado em importações subsequentes do usuário. Ele poderá ser reconfigurado se o administrador quiser um mapeamento diferente para importar usuários.
...
* Certifique-se de que a UUID, o endereço de email e o nome do funcionário sejam exclusivos em várias integrações do Workday. Valores incorretos podem resultar em uma falha de conexão.
* O campo UUID preenchido pelo Workday não pode ser excluído por nenhum administrador de LMS voltado para cliente. Se quiser alterar o valor, entre em contato com o Gerente de aprendizado da Adobe ou com a equipe de suporte.
* A opção Remover usuários também pode não funcionar, já que o User Purge suporta apenas 50 usuários a serem removidos por execução. Tenha extrema cautela ao fazer upload dos usuários por meio das UUIDs.
...
>[!PREREQUISITES]
>
>Solicite ao administrador do Workday da sua organização que crie um usuário do sistema de integração (ISU) com as permissões conforme definido no documento ISU_Permissions. Faça download de uma cópia do link abaixo.

[Baixe uma cópia de segurança do usuário do sistema de integração (ISU).](assets/isu-permissions-v1.pdf) Veja o processo para integrar o conector do Workday com o Gerenciador de aprendizagem.

1. Na página inicial do Gerenciador de aprendizado, passe o mouse sobre o quadro do Workday. Será exibido um menu. Clique no item **[!UICONTROL Conectar]** no menu.
...
>[!NOTE]
>
>   O Gerenciador de aprendizagem usa a versão 40.1 da API Workday.
...
Selecione a **caixa de seleção Excluir trabalhadores contingentes** para impedir que os trabalhadores temporários disponíveis sob um gerente sejam importados.

O Workday tem quatro níveis de hierarquia, enquanto o Learning Manager tem dois níveis. Os quatro níveis do Workday são categoria de perfil de habilidade, perfil de habilidade, categoria do item de habilidade e item de habilidade. O nome da habilidade e o nível do Gerenciador de aprendizagem juntos são mapeados no Workday sob o item de habilidade.
...
wd:Employment_Data.wd:Worker_Job_Data.0.wd:Position_Data.wd:Business_Site_Summary_Data.wd:Nome
...
do mesmo conector do Workday. Caso os nomes de habilidades sejam os mesmos em duas contas de Gerente de aprendizado, eles serão mapeados para a mesma habilidade no Workday. Antes de atualizar a habilidade no Workday, se duas contas de Gerente de aprendizado estiverem utilizando a mesma conta do Workday, é recomendável atualizar os nomes das habilidades em todas as contas do Gerenciador de aprendizagem.

+++Habilidades do usuário - Configurar
...
*Configurar o relatório de habilidades do usuário*
...
O administrador de integração pode escolher atributos do miniOrange e mapeá-los para os atributos agrupáveis correspondentes do Gerenciador de aprendizagem. Uma vez que o mapeamento é concluído, este mesmo mapeamento é usado em importações subsequentes do usuário. Ele poderá ser reconfigurado se o administrador quiser um mapeamento diferente para importar usuários.

#### Importação automatizada de usuário

O processo de importação do usuário permite que o administrador do Gerenciador de aprendizagem busque detalhes do funcionário do miniOrange e importe-os para o Gerenciador de aprendizagem automaticamente.

#### Como filtrar usuários

O administrador do Learning Manager pode aplicar filtros aos usuários antes de importá-los. Por exemplo, o administrador do Learning Manager pode importar todos os usuários da hierarquia sob um ou mais gerentes específicos.

Para configurar o conector do miniOrange, entre em contato com a equipe do Learning Manager CSM.

### Configurar o conector do miniOrange {#configureminiorangeconnector}

1. Na página inicial do Gerenciador de aprendizado, passe o mouse sobre a miniatura do miniOrange. Será exibido um menu. Clique  **[!UICONTROL na opção Conectar]** no menu.

   ![](assets/miniorange-tile.png)

   *Bloco do conector do miniOrange*

1. Clique **[!UICONTROL em Conectar]** para configurar uma nova conexão. A página do conector do miniOrange é exibida. Insira os detalhes da conta que quer mapear.

   ![](assets/establish-connection.png)

   *Criar uma conexão*

1. Se você deseja importar o usuário do miniOrange diretamente como um usuário interno do Gerenciador de aprendizagem, use a opção **[!UICONTROL Importar usuários]** internos.

   ![](assets/import-users.png)

   *Importar usuários internos*

1. No lado esquerdo da página de mapeamento você pode ver as colunas do Gerente de aprendizado e, no lado direito, as colunas do miniOnage. Selecione o nome da coluna apropriada que faz mapeamento para o nome de coluna do Gerenciador de aprendizado.
...
1. Para ver e editar a fonte de dados, como um administrador, clique em **[!UICONTROL Configurações > Fonte de dados]**.
   ...
   A origem do miniOrange estabelecida será listada. Se precisar editar o filtro, clique **[!UICONTROL em Editar]**.
...
## Conector Zoom {#zoom-connector}

Você pode integrar o Gerenciador de aprendizagem aos conectores Zoom e usá-los para hospedar as classes.  O conector permite configurar reuniões/classes de videoconferência com os alunos.

Para configurar e usar o conector, siga estas etapas.

1. Na página inicial do Gerenciador de aprendizado, passe o mouse sobre a miniatura Zoom. Será exibido um menu. Clique  **[!UICONTROL na opção Conectar]** no menu.

   <!-- ![](assets/connectors.png)

   *Zoom connector tile* -->

1. A página do conector Zoom é aberta. Insira os detalhes da conta nos campos específicos para integrar e sincronizar o feed do usuário. Você pode obter os detalhes com o administrador da sua conta do conector.
...
1. Depois que a conexão é estabelecida, como autor, crie um curso de vc com o Zoom como sistema de conferência.

   <!-- ![](assets/vc.jpg)
   
   *Create a VC course* -->

1. Os administradores, os gerentes e os alunos podem inscrever alunos no curso criado. Ao ser inscrito, o aluno receberá um e-mail. O aluno pode fazer logon na sua conta do Learning Manager para visualizar os detalhes do programa e realizar o curso.
...
### Criar um aplicativo OAuth de servidor para servidor de zoom

Ao criar um aplicativo OAuth de zoom servidor para servidor para ser usado no Adobe Learning Manager, você deve adicionar escopos exigidos pelo Adobe Learning Manager ao criar a conexão.
...
* Exibir as informações dos usuários e gerenciar usuários `/user:write:admin`

## Conector do Box {#boxconnector}

Com o conector do Box, é possível integrar o Learning Manager com sistemas externos arbitrários para automatizar a sincronização dos dados. A expectativa é de que os sistemas externos possam exportar os dados em um formato CSV e colocá-los na pasta apropriada da conta do Learning Manager Box. Os recursos do conector do Box são os seguintes:

Você também pode usar o conector FTP para migrar dados, importar o usuário e exportar dados. Para mais informações, consulte [Conector FTP do Learning Manager.](connectors.md#main-pars_header_1427405935)
...
O administrador de integração pode escolher colunas de CSV e mapeá-las para os atributos agrupáveis do Learning Manager. Este mapeamento é uma tentativa única. Uma vez que o mapeamento está feito, este mesmo mapeamento é usado em importações subsequentes do usuário. O mapeamento poderá ser reconfigurado se o administrador quiser um mapeamento diferente para importar usuários.
...
Saiba mais sobre o processo para integrar o conector do Box com o Learning Manager.

1. Na página inicial do Gerenciador de aprendizado, passe o mouse sobre a miniatura do Box. Será exibido um menu. Clique no item Conectar no menu.

   ![](assets/screen-shot-2017-10-25at54426pm.png)

   *Conectar-se ao Box*

1. É exibida uma caixa de diálogo solicitando que você insira a ID de e-mail. Forneça a ID de e-mail da pessoa responsável por gerenciar a conta do Box do Gerenciador de aprendizado da organização. Clique em Conectar após fornecer a id de e-mail.
1. O Learning Manager envia um e-mail solicitando que o usuário redefina a senha antes de acessar o Box pela primeira vez. O usuário deve redefinir a senha e usá-la para acessar a conta do Box do Gerenciador de aprendizado.

   >[!NOTE]
   >
   >Somente uma conta do Learning Manager Box pode ser criada para uma conta de Gerenciador de aprendizado.
...
Uma vez que uma conexão é estabelecida com sucesso, você pode mapear as colunas dos arquivos CSV que são colocados na pasta do Box para os atributos correspondentes do Gerenciador de aprendizagem. Essa etapa é obrigatória.
...
1. Clique em Salvar depois de concluir o mapeamento.

   O conector está pronto para uso. A conta configurada aparece como uma fonte de dados no aplicativo do administrador para que o administrador agende a importação ou sincronização sob demanda.

+++

+++Relatório de atividade da xAPI
...
1. Na página de configuração, selecione uma configuração existente disponível na lista de configurações para importar instruções xAPI do CSV. Clique em Editar ou Ad **um novo link de configuração** para navegar até a página Importar instruções xAPI-Configuration-Source File.
...
+++Usar o conector do Learning Manager Box
...
1. Os arquivos CSV dos sistemas externos devem ser colocados no seguinte caminho:
   ...
   >[!NOTE]
   >
   >Na versão de julho de 2016, é permitido importar somente usuários. Portanto, para usar o conector do Box, certifique-se de que os arquivos CSV sejam colocados na seguinte pasta:
...
Os arquivos exportados automaticamente estão presentes no local **Início/Exportação/&#42;Box_location&#42;**

Os arquivos exportados automaticamente são disponibilizados com o título skill_achievements_**dama data &#42;_até_&#42;&#42;.csv&#42;**

>[!NOTE]
>
>O cliente gerencia as permissões de acesso e o conteúdo na pasta do Box compartilhada pela equipe do Gerenciador de aprendizagem.  Além disso, o conteúdo na pasta seria armazenado fisicamente na região de Frankfurt.
...
>[!NOTE]
>
>Você obterá as IDs de lo exclusivas de todos os cursos importados do conector do LinkedIn Learning para o Adobe Learning Manager.

>[!NOTE]
>
>O tempo de aprendizado gasto nos cursos do LinkedIn Learning é comunicado pelo conteúdo do LinkedIn/plataforma do LinkedIn para a plataforma de aprendizado do Learning Manager. Se o LinkedIn Learning não enviar o tempo de aprendizado, ele não poderá ser gravado pela nossa plataforma de aprendizado. Nesse caso, o tempo de aprendizagem exibido pelo Gerenciador de aprendizado é zero.
...
1. Clique **[!UICONTROL no administrador]** do painel de navegação superior.
...
1. Clique **[!UICONTROL em Configurações]** de inicialização de conteúdo do LMS para expandir suas configurações.
...
1. No painel do administrador de integração, clique [!UICONTROL em LinkedIn Learning]. As opções Introdução, Conectar e Gerenciar conexões serão exibidas.
1. Se estiver configurando o conector do LinkedIn Learning pela primeira vez, clique [!UICONTROL em Conectar].
...
1. Na página de conexão, especifique um nome para o conector. Insira a chave do aplicativo e a chave secreta da sua conexão.
   ...
   >[!NOTE]
   >
   >O administrador da empresa pode gerar um novo aplicativo no portal do administrador do LinkedIn Learning para obter a chave do aplicativo e a chave secreta.

1. Clique em **[!UICONTROL Salvar]**.

   A configuração é salva e a conexão do LinkedIn Learning à sua conta é adicionada. Agora você pode clicar **[!UICONTROL em Gerenciar conexões]** na página inicial e editar sua configuração a qualquer momento.

1. Se já tiver uma conexão estabelecida, clique **[!UICONTROL em Gerenciar conexões]** para ver todas as conexões.
...
1. No painel esquerdo, clique em Configurar. Siga um destes procedimentos:
   ...
   * Exiba ou edite os detalhes de sua conta e o agendamento de sincronização nesta janela. Selecione a **[!UICONTROL caixa de seleção Ativar conexão]** se desejar ativar essa conta.
   * Clique **[!UICONTROL em Editar]** e edite suas credenciais. Para desfazer suas atualizações neste campo clique em Redefinir.
   * Clique **[!UICONTROL em Ativar agendamento]** para agendar a sincronização. Insira a hora e data de início e, em seguida, insira a frequência do agendamento de sincronização em dias. Por exemplo, habilitar a sincronização a cada três dias.

   Clique em **[!UICONTROL Salvar]** para salvar as alterações.

1. No painel esquerdo, clique **[!UICONTROL em Execução]** por demanda. Essa opção permite importar feeds do usuário e outros dados relevantes do LinkedIn. Insira a data de início para a execução sob demanda e clique em Executar para executar a sincronização. Todos os dados a partir da data de início até o presente serão importados.

   * Você pode clicar **[!UICONTROL em Desabilitar acesso]** ao Gerenciador de aprendizagem durante a execução quando o aplicativo tiver um tempo de inatividade durante a sincronização.
   * Se clicar **[!UICONTROL em Ativar acesso]** ao Gerenciador de aprendizagem durante a execução, não haverá interrupção no serviço durante a sincronização.
...
Use a integração com esse conector para aproveitar as contas existentes do Power BI para analisar e visualizar dados de aprendizado do Gerenciador de aprendizagem no Power BI. Durante a configuração, o administrador de integração pode configurar a área de trabalho do Power BI para que seja preenchida gradualmente com dois conjuntos de dados ao vivo: relatórios de transcrição do aluno e de habilidade do usuário. Você pode usar todos os recursos e o poder do Power BI para desenvolver, implantar e distribuir painéis personalizados como preferirem em suas empresas.

### Como configurar o conector {#configuringtheconnector}

Para configurar o conector, na página Conectores, passe o **[!UICONTROL mouse sobre o**[!UICONTROL  bloco Power BI ]**e clique**[!UICONTROL  em Conectar ]**.]** A página Power BI é aberta. Para estabelecer uma conexão, forneça a ID do cliente do aplicativo, o segredo do cliente do aplicativo, o nome do inquilino e a ID da área de trabalho (opcional). Para obter essas credenciais, siga estas etapas.
...
1. Clique **[!UICONTROL em Incorporar para sua organização]** e faça logon em sua conta da Microsoft.
1. Digite o nome do aplicativo.
1. Na seção Tipo de aplicativo, selecione a opção Aplicativo da Web do servidor.
1. **[!UICONTROL Na seção Redirecionar URL]**, selecione a opção **Usar um URL** personalizado (Escolha esta opção se souber o URL do aplicativo de destino). Insira o seguinte URL:

   `https://learningmanager.adobe.com/ctr/app/azure/_callback` (atualize o domínio com base no ambiente)
...
1. Clique **[!UICONTROL em Registrar aplicativo]** e armazene a ID do cliente e o Segredo do cliente.

>[!NOTE]
>
>Se desejar autorizar a conexão novamente, você deverá criar outro aplicativo Power e especificar o URL de redirecionamento renomeado.
...
**Configuração de exportação**: se quiser agendar a extração do relatório. Selecione a **[!UICONTROL caixa de seleção Ativar agendamento]** e especifique a data e hora de início. Também é possível especificar o intervalo no qual deseja que o relatório seja gerado e enviado.

![](assets/power-bi-configureuserskillpage.png)

*Configuração de exportação para agendar o relatório*

**Exportar sob demanda: é** possível especificar a data de início e exportar o relatório usando a opção. O relatório é extraído da data inserida até o presente.
...
Na página Recursos de PowerBI-xAPI, clique **[!UICONTROL em Exportar relatório]** de atividade do xAPI.

![](assets/powerbi-dashboard.png)
*PowerBI - Exportar relatório de atividade de xAPI*
...
![](assets/configure-schedule.png)
*Configuração de exportação de xAPI*
...
*Exportação de xAPI sob demanda*
...
Selecione **Status da execução** para ver o resumo de todas as tarefas em ordem cronológica. O sinal de aviso indica falhas durante a execução. Você pode baixar relatórios de erro como **CSV** clicando no link do relatório de erro.

![](assets/execution-status.png)
*Status da execução da exportação do xAPI*

### Relatórios unificados {#unified-reports}

O Gerenciador de aprendizado fornece uma maneira de criar a exportação com uma combinação de relatórios como dados do usuário, transcrição do aluno, gamificação, relatórios de feedback e muito mais, como um conjunto de dados para o Power BI.
...
*Relatórios unificados do Power BI*
...
* **Não iniciado:** exclua os registros registrados na faixa de datas especificadas, mas não começaram ao gerar o relatório.

* **Não inscrito:** Inclua todos os registros não inscritos no intervalo de datas.

![](assets/lt-filters.png)
*Status de filtro das transcrições de aprendizado*
...
O Gerenciador de aprendizado também fornece modelos de Power BI prontos. Esses modelos fornecem uma melhor capacidade de análise para os administradores de conta do Adobe Learning Manager.

Você pode baixar os modelos, exportar relatórios relevantes e plotar relatórios usando esses modelos disponíveis facilmente.

![](assets/download-power-bi-template.png)
*Baixar modelos do Power BI*

Isso permite que os usuários baixem esses modelos e usem-os no aplicativo Power BI e personalizem-nos ainda mais, e fazer com que seus relatórios contem uma história interessante.

[**Baixar modelos**](https://documentcloud.adobe.com/link/track?uri=urn:aaid:scds:US:842bb6a2-cd7d-4c3d-b968-da38bc1cc18a)
...
#### Administrador: transcrições do aprendizado e relatório unificado

**Conexões existentes**

Se a opção Caminho do aprendizado estiver desativada na conta de Administrador, nenhuma linha e coluna são adicionadas nos relatórios.

Se a opção Caminho do aprendizado estiver habilitada na conta de Administrador, o relatório conterá o caminho de aprendizagem do tipo coluna (nível superior) para todos os alunos inscritos em um caminho de aprendizagem.

**Novas conexões**

Se a opção Caminho do aprendizado estiver desativada na conta de Administrador, o relatório de treinamento será composto das seguintes colunas:
...
Na coluna Tipo, Programa de aprendizado será renomeado como Caminho de aprendizado. Para conexões existentes, não haverá alterações. Entretanto, para novas conexões, as alterações serão refletidas após 30 dias.
...
Se a opção Caminho do aprendizado estiver habilitada na conta de Administrador, o relatório conterá a coluna &quot;Tipo&quot;. A coluna contém o novo valor &quot;Caminho de aprendizagem (nível mais alto), quando aplicável&quot;.
...
Para configurar seu próprio FTP, faça logon como administrador de integração e clique **[!UICONTROL em FTP]** personalizado > **[!UICONTROL Connect]**.

Há dois tipos de autenticação:

![](assets/custom-ftp-authenticationoptions.png)
*Opções personalizadas de autenticação por FTP*
...
* **Certificação:** se o FTP do cliente suportar a autenticação de certificado, eles podem escolher essa opção. Depois de clicar em Gerar chave SSH, a chave SSH será baixada para o computador local. Quando você abre o arquivo, a tecla tem a seguinte aparência:

![](assets/ssh-public-key.png)
*Chave pública SSH*

Você deve colocar essa chave pública no servidor FTP antes de adicionar os detalhes abaixo. Depois de definir a chave especificada como a chave pública do FTP, forneça o url do domínio ftp e o nome do usuário e clique no **botão Conectar** para configurar a conexão.

Depois que a configuração da conexão é feita, as pastas automaticamente para importação e exportação são criadas no local do ftp. Depois que a funcionalidade de importação/exportação for fornecida por FTP personalizado.
...
* Faça logon em seu portal Azure usando este URL:  [https://portal.azure.com/](https://portal.azure.com/) antes de registrar seu aplicativo.
* Abra o Azure Active Directory.

## Etapas para registrar seu aplicativo {#stepstoregisteryourapplication}

* Clique em Azure Active Directory. Clique **[!UICONTROL em Adicionar]** > **[!UICONTROL registro do aplicativo]**.
...
* Procure *diretório* e selecione **Ler dados do diretório**.
  ...
  *Selecionar dados do diretório Ler*
...
1. Faça logon **no portal.azure.com**. Os valores serão preenchidos no ID do inquilino e nos campos Domínio principal.
...
1. Se quiser importar o usuário do ADFS diretamente como um usuário interno do Gerenciador de aprendizado, use a opção Importar usuários internos.

   ![](assets/adfs3.jpg)

   *Importar usuário para o Gerenciador de aprendizado*

1. No lado esquerdo da página de mapeamento você pode ver as colunas do Gerente de aprendizado e, no lado direito, as colunas do ADFS. Selecione o nome da coluna apropriada que faz mapeamento para o nome de coluna do Gerenciador de aprendizado.

   ![](assets/adfs4.jpg)

   *Mapear atributos*

1. Para exibir e editar a fonte de dados, como administrador, clique **[!UICONTROL em Configurações]** > **[!UICONTROL fonte]** de dados.

   A fonte do ADFS estabelecida será listada. Se precisar editar o filtro, clique **[!UICONTROL em Editar]**.

   ![](assets/datasource.jpg)
   *Configuração da fonte de dados*

1. Você receberá uma notificação após a conclusão da importação. Para exibir ou editar o registro de importação, clique **[!UICONTROL em Usuários]** > **[!UICONTROL Importar registro]**.

### Excluir uma conexão {#Deleteaconnection-1}

Para excluir uma conexão de miniOrange estabelecida, siga essas etapas.
...
1. Clique **[!UICONTROL em Integrar]**.

   Depois de autenticar a ID de e-mail, o Gerenciador de aprendizagem exibe a mensagem como o Connect é integrado com êxito. Você pode começar a visualizar seus cursos em sala de aula virtual usando o Adobe Connect automaticamente.
...
No logon do administrador, clique no nome do curso de sala de aula virtual. Clique em Instâncias no painel esquerdo e nos Detalhes da sessão.  Clique no ícone Editar, no canto direito da página Detalhes da sessão, para adicionar as informações da sessão.
...
Importe os dados do quiz no Learning Manager e integre-os ao fluxo de trabalho de relatório existente para que os usuários do Gerenciador de aprendizagem possam obter dados do quiz, respostas do usuário e pontuações das sessões do Adobe Connect no relatório, como a forma como ele está disponível para módulos em ritmo individualizado terem questionários.
...
**Gerenciador de aprendizagem - Administrador**
...
* Em seguida, o administrador escolhe o curso do Gerenciador de aprendizagem criado anteriormente. O administrador vê o seguinte:
...
O Learning Manager integra-se com o Marketo Engage, um software de automação de marketing que ajuda a executar campanhas de marketing.

O Conector do Marketo Engage foi projetado para adicionar (ou atualizar) leads no banco de dados do Marketo Engage quando um novo usuário é adicionado à conta do Gerenciador de aprendizagem. Ele também associa os comportamentos de aprendizagem do usuário no Gerenciador de aprendizagem (inscrição no curso, conclusão do curso, atribuição de habilidade e realização da habilidade) como objetos personalizados aos leads correspondentes no Marketo Engage. Isso permite que um profissionais de marketing use essas informações para direcionar o público-alvo com base em seus comportamentos de aprendizagem capturados do Gerenciador de aprendizagem e use recursos do Marketo Engage como &quot;Listas inteligentes&quot;.
...
Para que o Gerenciador de aprendizagem se integre à sua conta do Marketo, sua conta marketo precisa ter a capacidade de criar esquemas por APIs.
...
* ID do cliente
* Segredo do cliente
* Marketo Engajar domínio

![](assets/marketo-creds.png)

*Insira credenciais para o Marketo*

>[!NOTE]
>
>Você pode obter a ID do cliente e o segredo do aplicativo Marketo Engage. No aplicativo do Marketo, você pode obter a ID do cliente e o **segredo na seção LaunchPoint** e o Domínio do Marketo na **seção WebServices** .

**Na seção Relatórios** unificados da conexão do Markeo Engage no aplicativo do Gerenciador de aprendizagem, você pode criar campanhas com base no seguinte:

* Um novo usuário é adicionado ao Gerenciador de aprendizagem
...
O mapeamento de coluna é usado para criar o banco de dados de leads. Os leads são usuários que você exportou do Relatório de usuário.

Os campos do Relatório do usuário são listados na coluna Adobe Learning Manager. Os campos na coluna Marketo são os que o Marketo fornece. Usando ambas as colunas, você pode mapear qualquer campo do Gerenciador de aprendizagem a partir do Marketo. Na coluna Gerente de aprendizado, você participa de uma coluna relacionada no Marketo. Depois de unir as colunas, um banco de dados de leads é criado.

Em seguida, você pode exibir todos os usuários exportados no Marketo.

Na seção **Objetos personalizados do Marketo** no aplicativo Marketo, você pode ver que todos os três relatórios, Transcrição do aluno, Habilidade do usuário e Relatório do usuário, estão presentes. Esses relatórios têm a string **&quot;cp_&quot;** preparada para cada um. Cada novo usuário que é exportado para o Marketo é considerado um lead.

### Eventos

Exportar dados de eventos do Gerenciador de aprendizagem para uma instância do Marketo Engage. Selecione os eventos a serem exportados para o banco de dados do Marketo Engage sob demanda ou em uma programação.
...
**Permitir que o Microsoft Teams autentique os alunos ao entrar na sala de aula virtual**
...
**Usar sincronização automatizada de conclusão do usuário**
...
Para obter mais informações, consulte  [**o conector Instalar do Microsoft Teams no Adobe Learning Manager**](install-microsoft-teams-connector.md).

## Experiência não registrada

A experiência não registrada permite que você crie uma experiência em tempo real para usuários não conectados. Por exemplo, uma experiência não registrada serve como uma página inicial para campanhas de marketing para incentivar cadastre-se.

A experiência não registrada no Adobe Learning Manager pode ser configurada usando o **[!UICONTROL conector do Acesso]** aos dados de treinamento. O conector fornece as seguintes ofertas:
...
A oferta padrão é criar a versão nativa do Adobe Learning Manager. Os usuários podem criar uma experiência sem cabeça somente para demonstração e não registrada. A demonstração de que a experiência sem cabeça é incalculável e não deve ser usada em um ambiente de produção.

**Oferta Premium**

A oferta Premium ajuda os usuários a criarem uma interface sem cabeça, configurada pelo **[!UICONTROL conector de acesso]** aos dados de treinamento. Isso permite que os usuários obtenham dados em tempo real no curso e no caminho do aprendizado, como nome, descrição, autor, habilidades, duração etc. Para cenários de aprendizado mistos, você também obtém limites de licenças em tempo real, assentos ocupados, limites de listas de espera e contagem de listas de espera. Os clientes podem usar essas APIs para criar recursos de pesquisa e filtro e um resumo completo do curso para alunos não conectados.

Os clientes podem comprar um plano Premium para criar essa experiência não registrada altamente dimensionável.

>[!NOTE]
>
>Entre em contato com a equipe de suporte ou a CSM para comprar o plano premium.

Depois que um usuário comprar um plano, a equipe da CSM ativará o plano premium para ele. Com o conector do Acesso aos dados de treinamento, os usuários podem configurar uma experiência não registrada com os recursos mencionados anteriormente.

### Conector de acesso aos dados de treinamento

>[!IMPORTANT]
>
>Essa funcionalidade específica está disponível somente se o Adobe Learning Manager for vendido como um complemento ao Adobe Experience Manager. Os dados do curso estariam obsoletos em 24 horas.

>[!NOTE]
>
>A seção destaca como a infraestrutura funciona, mas para criar uma experiência não registrada ou não registrada no AEM, entre em contato conosco. Sugeriremos a base de abordagem certa para o seu caso de uso. Essa funcionalidade não está disponível atualmente como autoatendimento.

O **[!UICONTROL conector do Acesso]** aos dados de treinamento permite criar uma experiência sem cabeça. Essa experiência pode ser independente ou uma interface de usuário personalizada com base nos sites do AEM. Ajuda a recuperar e exibir informações de treinamento para os alunos e permite pesquisar e filtrar. Quando o conector de dados for ativado, um conjunto de APIs públicas estará disponível para criar a interface, onde as informações do caminho de aprendizado/curso serão exibidas aos alunos.

#### Configurar o conector

Use o **[!UICONTROL conector do Acesso]** a dados de treinamento para integrar sua conta do Adobe Learning Manager aos sistemas de armazenamento e pesquisa de dados. Isso ajuda sua interface baseada em sites do AEM a obter dados de treinamento, exibir páginas da Web e oferecer melhores opções de pesquisa para os alunos.

Exporte metadados de treinamento do Adobe Learning Manager para os serviços de recuperação e pesquisa de dados usando as APIs. Você também pode criar um cronograma para automatizar essas exportações.

Para configurar o conector de acesso aos dados de treinamento, siga estas etapas:

1. No aplicativo do administrador de integração, selecione **[!UICONTROL Acesso]** aos dados de treinamento > **[!UICONTROL Introdução]**.
1. Selecione **[!UICONTROL Próximo]** na **[!UICONTROL página Introdução]** .
1. Digite o nome da conexão e permita os domínios listados.

   ![](assets/connection-name-and-domain-name.png)Nome da conexão de tipo e nome de domínio

1. Selecione o **[!UICONTROL tipo de interface]** entre as seguintes opções:

   * **[!UICONTROL Gerenciador]** de aprendizagem nativo: esta é a oferta padrão, que está disponível apenas para a interface nativa.
***[!UICONTROL Interfaces]** sem cabeça: esta é a oferta premium que expõe APIs para criar uma experiência não registrada.

   ![](assets/types-of-interface.png)Tipos de interface

1. Selecione **[!UICONTROL Conectar]**. O URL de base e o URL de CDN serão gerados automaticamente.
Você pode usar esses URLs para recuperar os dados usando APIs.

   >[!NOTE]
   >
   >Os clientes que usam a oferta Premium receberão um URL diferente dos que usam a oferta padrão.


1. Selecione **[!UICONTROL Exportar metadados]** de treinamento na página do conector.
1. Selecione **[!UICONTROL Ativar exportação]** de metadados de treinamento usando esta conexão para exportar os dados de treinamento.
1. Depois de habilitar a conexão, as imagens de todos os cursos, caminhos de aprendizado e certificados são migradas para o CDN.
1. Exporte os metadados dos cursos, caminhos de aprendizagem e certificados para o serviço de pesquisa e recuperação.
1. Você pode agendar a exportação de metadados selecionando a opção Ativar agendamento. A programação ocorrerá automaticamente a cada 3 horas do plano premium.
1. Para um relatório sob demanda, vá até **[!UICONTROL Sob demanda]**, selecione a **[!UICONTROL data de]** início e **[!UICONTROL clique]** em Executar.
É possível verificar o status da execução do relatório na página Status]**da**[!UICONTROL  execução.

### Criar site no AEM

**Pré-requisito:** instale o pacote do AEM no repositório **](https://github.com/adobe/adobe-learning-manager-reference-site/releases/tag/1.0.0) do[** GitHub.
...
>[!NOTE]
>
>Essa funcionalidade específica está disponível somente se o Adobe Learning Manager for vendido como um complemento ao Adobe Experience Manager.
...
1. Ative o  [RabbitMq](https://devdocs.magento.com/cloud/project/services-rabbit.html) ou qualquer outro corretor de mensagens.
1. Ativar  [CRON](https://devdocs.magento.com/cloud/env/variables-deploy.html#cron_consumer_runner).
...
1. Habilite todas as APIs assíncronas na página. Como pode haver muitos dados, a exportação ocorre de forma assíncrona. As APIs do Adobe Commerce são chamadas de carga de solicitação enviada. A solicitação empurra as mensagens para uma fila e há um consumidor nessa fila, que processa essas mensagens e cria produtos no lado do comércio. O Adobe Commerce não fornece esse processamento assíncrono por padrão. É por isso que você deve ativar esta opção.
1. Adicione um link para retornar ao ALM na página de sucesso do pagamento. Esse URL de retorno deve ser configurado no Adobe Commerce. O URL a ser usado para o link. -  `https://learningmanager.adobe.com/app/learner#/postPayment`
1. Altere a indexação de &quot;Ao salvar&quot; para &quot;Programado&quot;.  Para obter mais informações, consulte esta  [fonte KB](https://support.magento.com/hc/en-us/articles/360040227191).
1. Aplique os seguintes patches. Para obter mais informações, consulte  [Aplicar correções](https://devdocs.magento.com/cloud/project/project-patch.html).
1. Configure com rapidez.  Rapidamente é necessário para o Adobe Commerce em infraestrutura em nuvem e é usado em ambientes de Teste e Produção. Para mais informações, consulte [Configurar o Fastly](https://devdocs.magento.com/cloud/cdn/configure-fastly.html).

### Configurar o conector

Como administrador de integração, no conector do Adobe Commerce, clique em **[!UICONTROL Conectar]**.

Na página de configuração, insira os seguintes detalhes. Esses detalhes, as chaves de autorização, estão disponíveis no Adobe Commerce. Assim que você criar uma integração no Adobe Commerce, as credenciais ficarão disponíveis lá.

![](assets/adobe-commerce-configuration.png)
*Configurar o Adobe Commerce Connector*
...
>[!NOTE]
>
>Na versão de março de 2024 do Adobe Learning Manager, apresentamos suporte para [o Adobe Commerce 2.4.6](https://experienceleague.adobe.com/docs/commerce-operations/release/notes/adobe-commerce/2-4-6.html?lang=en).


1. Clique **[!UICONTROL em Exportar metadados]** de treinamento > **[!UICONTROL sob demanda]**.

1. Selecione as datas.

1. Clique em **[!UICONTROL Executar]**. Após a execução bem-sucedida, todos os cursos ou caminhos de aprendizado com preços definidos serão movidos para o Adobe Commerce. Em seguida, o aluno pode adquirir o curso no Gerenciador de aprendizado.

### Gerente de aprendizagem nativa com o Adobe Commerce

#### Aluno

Como aluno, você deve estar conectado para comprar um curso, certificado ou caminho de aprendizado.

Para comprar o curso, clique em Comprar agora. Você é redirecionado para o Adobe Commerce para concluir a compra. Assim que o pagamento for processado, você poderá ver uma mensagem que solicita que você retorne ao Gerente de aprendizado e inicie o curso. Você também deve fazer logon no Adobe Commerce separadamente para concluir a compra.
...
Além disso, também é possível ativar/desativar emails da Adobe Commerce.

### Sites do AEM com o Adobe Commerce
...
O administrador habilita a caixa de seleção, **Ativar preço para treinamentos**, nas **Configurações > Geral** no aplicativo de Administrador. Se a opção estiver ativada, os autores podem especificar os preços dos treinamentos. Quando você adiciona uma conexão do Adobe Commerce, essa caixa de seleção é automaticamente selecionada e aplicada.
...
* Os usuários podem gerar receita vendendo cursos no Adobe Learning Manager ou em uma interface de aprendizado sem cabeça.
* Habilite a integração do Adobe Commerce à plataforma para vender cursos usando o aplicativo nativo do Learning Manager e o AEM.
* Permita que os clientes do Gerente de aprendizado ofereçam aprendizado formal na forma de cursos pagos.
* Permita que os alunos visualizem os cursos antes de decidirem comprar o treinamento.
...
>[!NOTE]
>
>O Adobe Learning Manager e o Adobe Commerce têm logins diferentes.

### AEM

Nesse modo, um aluno faz o curso de um site baseado no AEM, que é criado usando modelos e componentes baseados no AEM.

No site do AEM, o aluno tem suporte para carrinho de compras, botão Adicionar ao carrinho, excluir cursos do carrinho de compras e assim por diante.
...
A sincronização entre o Learning Manager e o Adobe Commerce acontece duas vezes ao dia. Depois que o administrador habilita uma conta de e-commerce, a opção **Ativar exportação de metadados de treinamento usando essa opção de conexão** , quando ativada, armazena as imagens do curso, do caminho de aprendizado e dos certificados em um CDN público.

Se os dados permanecerem não sincronizados, as informações de preço não serão exibidas para um aluno.

No Gerenciador de aprendizagem nativo, se o e-commerce estiver ativado e a sincronização entre o Gerenciador de aprendizagem e o Adobe Commerce estiver concluída, os alunos poderão visualizar ou pesquisar treinamento gratuito ou pago.

Para o AEM, não existe Comprar agora, somente um **botão Adicionar ao carrinho** . Esse botão também permanece desabilitado se a sincronização não for executada.
...
+++Há alguma alteração no relatório de transcrição e treinamentos do aluno?
