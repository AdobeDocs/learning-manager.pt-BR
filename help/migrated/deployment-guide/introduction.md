...
Este guia fornece instruções passo a passo sobre como começar a usar o Learning Manager. Este documento também fornece informações detalhadas sobre configuração e instalação, em detalhes. Leia para saber como começar a usar o Learning Manager.
...
Após adquirir o Learning Manager, ative sua conta do Learning Manager usando a chave de licença recebida. Prossiga para as seguintes configurações, conforme indicado no seguinte visual:

![](assets/getting-started-withcaptivateprime.jpg)

## Configurar seu site no Learning Manager {#configureyoursiteincaptivateprime}

Antes de começar a adicionar e implementar objetos de aprendizado no Learning Manager, há algumas configurações-chave que são necessárias. Comece configurando seu site para adequá-lo à sua organização. A configuração do site compreende as seguintes etapas:

* Configurar marcas e logotipos para sua organização
* Configurar modelos de e-mail
* Configurações básicas da conta
* Configurações de feedback
* Definir as configurações do painel do aluno
...
* No painel esquerdo, clique em **Marca**.
* Na página Marcas, você pode configurar as seguintes opções clicando em **Editar** em relação à opção que você deseja modificar:
...
O Learning Manager permite alterar a aparência do seu site usando Temas. O aplicativo fornece os seguintes temas de cores para você escolher:
...
1. No painel de navegação esquerdo do Learning Manager, clique em **[!UICONTROL Marca]**.
1. No menu **Temas** clique em **[!UICONTROL Editar]**. O aplicativo permite que você escolha um novo tema. Ao selecionar um tema, você pode ver imediatamente os esquemas de cores usados para os elementos essenciais da interface.
...
1. Para redefinir os valores para o esquema de cores padrão do tema, clique em **[!UICONTROL Redefinir Tema]**. As cores dos elementos principais da interface do usuário são definidas como as opções padrão para o tema escolhido.
1. Depois de escolher o tema, clique em **[!UICONTROL Mostrar dicas]** para exibir os rótulos ou dicas na visualização.
   ...
   Observe uma apresentação de slides com várias imagens no **Temas** seção. Esta apresentação de slides permite visualizar instantaneamente o tema ou o esquema de cores. Você pode visualizar imediatamente páginas selecionadas, como a página inicial, o painel do aluno e assim por diante.

1. Se quiser visualizar as alterações em um navegador, clique em **[!UICONTROL Visualização ao vivo]**. Uma janela pop-up Visualização dinâmica do tema é exibida, onde você pode modificar o esquema de cores ou continuar com as opções padrão. Para visualizar suas opções em um navegador, clique em **[!UICONTROL Visualizar]** nesta janela pop-up.

   ![](assets/setting-the-themesforyoursite-step6.png)

1. As opções escolhidas são aplicadas temporariamente ao seu site. Se quiser salvar o tema e as configurações de cor selecionadas, clique em **[!UICONTROL Aplicar]**.
...
* Modelos de e-mail gerais: esses e-mails são acionados para eventos genéricos. Por exemplo, uma notificação de boas-vindas quando um usuário faz logon pela primeira vez.
...
Você pode ativar e configurar qualquer uma dessas notificações por e-mail no painel Administrador. Para saber como definir modelos de e-mail, execute as seguintes etapas:
...
1. É possível modificar o modelo de e-mail padrão. Para fazer isso, clique no evento. Neste exemplo, clique em **[!UICONTROL Programa de aprendizado - Inscrito por Administrador/Gerente.]**
1. No menu **[!UICONTROL Visualização do modelo]** pop-up, observe que há duas guias: [!UICONTROL Aluno] e [!UICONTROL Gerente].
   ...
   Para cada uma dessas guias, clique no corpo do e-mail para modificar o conteúdo. Para salvar as alterações no modelo de email, clique em **[!UICONTROL Salvar]**.
...
1. Observe que não foi possível modificar o URL da conta ou a assinatura no modelo de e-mail. Para modificar o **[!UICONTROL URL da conta]** ou **[!UICONTROL Assinatura]**, clique no botão **[!UICONTROL Configurações]** guia. Nesta guia, você pode modificar o banner de e-mail, a assinatura de e-mail e o URL da conta.

   O link do URL da conta é mostrado em todos os emails, pouco antes da assinatura. Insira o URL de sua preferência e clique em **[!UICONTROL Salvar]**. Esse URL é visível somente para os usuários internos.
...
Além de definir modelos de e-mail em um nível global, como administrador, você também pode configurar modelos de e-mail para um objeto de aprendizado específico. Nesse caso, todas as alterações feitas no modelo de e-mail são aplicáveis somente a esse objeto de aprendizado.
...
1. Clique no curso, programa de aprendizado ou certificação para o qual deseja configurar o modelo de e-mail.
1. No painel esquerdo, clique em **[!UICONTROL ** Modelos de e-mail **.]** O sistema exibe um ****[!UICONTROL Visualização do modelo]**** pop-up.
...
### Restringir usuários de receber emails {#restrictusersfromreceivingemails}

Como administrador, você pode selecionar quem receberá ou não e-mails do Learning Manager. Você pode fazer isso usando a guia ****[!UICONTROL Usuário restrito]**** na caixa de diálogo ****[!UICONTROL Configurações]** **guia. Os usuários podem ser adicionados a esta lista usando seu nome, ID de email ou ID de usuário exclusiva. Os usuários listados nessa opção ficarão restritos ao recebimento de qualquer comunicação por e-mail a partir do Learning Manager.

## Defina as configurações da sua conta {#configureyouraccountsettings}
...
1. Configure as seguintes opções:
   ...
   * **Localidade**: selecione o idioma de sua escolha. Se você alterar o idioma nesse campo, a alteração será aplicada a todos os usuários que usam esse aplicativo. No entanto, individualmente, cada usuário pode modificar o idioma de preferência.
   * **O exercício tem início em**: selecione o mês em que o exercício financeiro da sua organização começa.
...
1. Para configurar as outras perguntas para o seu questionário de feedback, clique nas perguntas da seção ****[!UICONTROL Cursos de ritmo individualizado]****, ou ****[!UICONTROL Cursos em sala de aula]****. Quando você clica em uma pergunta, o sistema permite editar as perguntas padrão.
...
1. Defina as configurações de lembrete preenchendo as seguintes opções:
   ...
   * **Dias após a conclusão**: especifique o número de dias após os quais você deseja enviar a solicitação de feedback. Este campo ficará visível somente se for selecionado ****[!UICONTROL Após a conclusão do curso]****.
...
1. Clique na marca de seleção para salvar as configurações do lembrete.
...
Depois de finalizar a pergunta de feedback e as configurações do lembrete, clique em ****[!UICONTROL Salvar]**** para aplicar suas configurações.
...
1. Selecione o **[!UICONTROL Tornar obrigatório]** se quiser tornar uma ou ambas as perguntas obrigatórias.
1. Selecione o **[!UICONTROL Mostrar questionário imediatamente após a conclusão do curso]** se você deseja que os alunos visualizem o questionário de feedback imediatamente após a conclusão do curso.

   ![](assets/configure-feedbackataninstancelevel-step7.png)

1. Para configurar o feedback de Alteração de comportamento N3 em um nível de instância, ****[!UICONTROL Habilitar]**** o Feedback N3. O aplicativo exibe uma pergunta predefinida e obrigatória e uma pergunta em branco onde você pode inserir uma pergunta de sua escolha.
...
1. Especifique a segunda pergunta para o gerente. Os gerentes podem fornecer uma resposta descritiva para essa pergunta.
1. Selecione o ****[!UICONTROL Tornar obrigatório]**** se quiser tornar a segunda pergunta obrigatória.
...
1. Na página Configurações gerais, você pode definir as seguintes opções:
   ...
   * **Mostrar eficácia do curso**: ative essa opção se deseja que os alunos vejam a eficácia de um curso no título do curso.
   ...
   * **Quadro de discussão**: ative essa opção se deseja que os alunos visualizem e participem de painéis de discussão para cursos. Se você habilitar a opção **Quadro de discussão** , os alunos e professores podem postar comentários nos cursos. No entanto, se as configurações no nível do curso indicarem que este recurso não está selecionado, as configurações no nível do curso prevalecem sobre as configurações do administrador.

   * **Opção Explorar habilidades**: ative essa opção se deseja que os alunos explorem habilidades de liderança e colegas.
   ...
   * **Mostrar Listagem de Catálogos**: ative essa opção se deseja que os alunos vejam todos os catálogos disponíveis. Essa opção ajuda os alunos a refinar suas listas de objetos de aprendizado.
...
1. Para aplicar as configurações padrão, clique em **[!UICONTROL Restaurar para padrão.]** Nesse caso, todos os widgets, exceto **[!UICONTROL Bem-vindo e Comunicados Adesivos]** são visíveis.
