...
>[!NOTE]
>
>Confira este [webinar](https://learningmanager.adobe.com/app/learner?accountId=98632#/course/9212121) para saber mais sobre os novos recursos nesta versão.
...
Exiba [habilidades de importação](administrators/feature-summary/import-skills-external-sources.md) para saber mais.
...
Exiba [várias marcas personalizadas](administrators/feature-summary/themes.md#multiple-branding) para obter mais informações.
...
Se um grande número de alunos estiver inscrito no curso, por exemplo, mais de 50.000, a lista de alunos não será exibida. Você pode pesquisar um aluno na barra de pesquisa *Pesquisar Alunos* ou selecionar o link **Baixar** na parte superior da barra de pesquisa para baixar a lista de alunos.

### Administrador - Página Alunos

Ao procurar qualquer usuário, as opções **Baixar aluno** e **Exportar** baixam o mesmo relatório. Enquanto isso, ao procurar um grupo de usuários, agora você pode baixar usuários filtrados desse grupo de usuários. Ao pesquisar um grupo de usuários,
a opção **Baixar lista de alunos** muda para **Baixar lista de alunos para grupo de usuários**. A opção **Exportar** baixa novamente a lista inteira.
...
Nas versões atuais do Adobe Learning Manager, se houver um grande número de usuários inscritos em uma certificação, você não poderá exibir os alunos não inscritos, pois o menu suspenso **Status** está desabilitado.

Nesta versão do Adobe Learning Manager, se o número de usuários inscritos for grande, o menu suspenso **Status** exibirá apenas duas opções: **Inscrito** e **Não inscrito**. A opção **Inscrito** está selecionada por padrão. Se você selecionar **Não inscrito**, será exibida a lista de alunos não inscritos.

#### Alterações no grupo de usuários

No caso de um Grupo de Usuários, se o número de usuários no Grupo for inferior a, por exemplo, 50.000, o menu suspenso **Status** exibirá todas as opções: Certificado, Atribuído e Vencimento.

Se o número de usuários em um Grupo de Usuários for grande, o menu suspenso **Status** exibirá apenas duas opções: **Inscrito** e **Não inscrito**, de acordo com o novo design.
...
Com base nas [configurações do administrador](administrators/feature-summary/classroom.md#classroom-settings), você, como autor, pode [criar, modificar e excluir locais](administrators/feature-summary/classroom.md#add-classroom-location).
...
Como administrador, você pode aplicar restrições a um autor para modificar ou excluir um local da sala de aula. Exiba [Configurações de sala de aula](administrators/feature-summary/classroom.md#classroom-settings) para obter mais informações.
...
Exiba [Planos de Aprendizado](administrators/feature-summary/learning-plans.md) para obter mais informações.
...
Exiba [status de envio](instructors/feature-summary/learners.md#filter-file-submissions) para obter mais informações.
...
Isso significa que a caixa de seleção **Habilitar** não foi selecionada na seção Reavaliação ao criar a lista de verificação. Se esta caixa de seleção estiver selecionada, você poderá ver o botão Exibir/Reavaliar na página Lista de verificação do professor.
...
Nas versões atuais do Adobe Learning Manager, um aluno é marcado como **Não participou** quando está inscrito em uma sessão de sala de aula virtual do Connect, mas não atendeu aos critérios de conclusão.
...
Para obter mais informações, consulte o White labelling no [aplicativo para dispositivos móveis da Adobe Learning Manager](white-label.md).
...
Se você executar uma migração para atualizar um curso existente ou um Plano de Aprendizado ou uma certificação, o curso ou Plano de Aprendizado ou a certificação com a **uniqueLOId** s será adicionado ao aplicativo Autor.

Ao migrar, você deve atualizar os valores de **uniqueLOId** nos CSVs do curso ou plano de aprendizado ou certificação, mesmo que seja uma coluna opcional.

Se a coluna **uniqueLoId** não for adicionada antes da execução da migração durante a atualização do curso ou plano de aprendizado existente ou certificação com **uniqueLOId** s, após a migração, os valores **uniqueLOId** serão substituídos por valores NULL.

>[!NOTE]
>
>A coluna **uniqueLoId** não se aplica ao CSV de Ajuda de Trabalho.
...
Baixe os CSVs do [Manual de migração](integration-admin/feature-summary/migration-manual.md#csv-specifications-and-sample-csvs).
...
Exiba [Conectores no Adobe Learning Manager](integration-admin/feature-summary/connectors.md) para obter mais informações sobre como configurar conectores.

### Alterações no relatório Acesso de logon

O relatório Login Access estará disponível apenas para os últimos cinco trimestres. Se algum administrador de integração solicitar o download Por demanda da Exportação unificada com **Acesso de logon** marcado, o Adobe Learning Manager exibirá uma mensagem de erro. No entanto, não há impacto em outros relatórios.
...
Se você tentar baixar o relatório do **Acesso ao Logon** que ultrapassa cinco trimestres, será exibida uma mensagem de erro. Uma mensagem de erro semelhante será exibida se você tentar baixar o relatório de **Auditoria de Usuário** por mais de seis meses.

### APIs obsoletas

Exiba [depreciações de APIs no Adobe Learning Manager](api-deprecations-list.md) para obter uma lista cumulativa de todas as APIs obsoletas no produto.
...
* O botão **Cancelar inscrição** não é exibido como esperado após a primeira inscrição, mas é exibido após uma atualização.
...
* No aplicativo móvel ou modo imersivo, após excluir ou desativar uma instância do curso e selecionar a próxima instância ativa, o botão **Registrar interesse** é exibido em vez de **Inscrever-se**.
