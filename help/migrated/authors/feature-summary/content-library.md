...
1. O Adobe Learning Manager é compatível com legendas para conteúdo de vídeo carregado no Adobe Learning Manager. Agora, os autores podem fazer upload do arquivo que contém legendas, junto com o arquivo de vídeo.
   ...
   As legendas aparecerão no Fluidic Player. As legendas também são compatíveis com os padrões [WCAG2.0](https://www.w3.org/TR/WCAG20/).
   ...
   *Adicionar um arquivo webvtt*
   ...
   Ao adicionar esse conteúdo a um curso e visualizá-lo como aluno, você pode ver as legendas no vídeo.
...
Se uma conta de vendedor compartilha um catálogo que contém os cursos, e os cursos contêm os módulos, o áudio ou o vídeo com as legendas, os cursos devem funcionar da mesma forma na conta de comprador.
...
Por exemplo, as agências desejam manter o controle total do conteúdo e alguém que ignora o conteúdo deve ter acesso a todo ele. Ao mesmo tempo, os criadores de conteúdo nas agências devem ter acesso apenas ao seu próprio conteúdo e, em alguns casos, ao conteúdo de outra pessoa.

A biblioteca de conteúdo com conteúdo existente (ou seja, o conteúdo carregado antes das pastas de conteúdo serem configuradas) é definida como **Pasta pública**. Esta pasta não pode ser descontinuada ou excluída. O conteúdo que faz parte da pasta pública é acessível a todos os tipos de autores. Depois que as pastas de conteúdo são configuradas, os autores padrão e os autores personalizados devem selecionar a pasta onde o conteúdo deve ser inserido, ao carregar o novo conteúdo.
...
Se você escolher **Público**, o conteúdo ficará visível para todos os autores. Por padrão, todo o conteúdo que existia na conta que não faz parte de nenhuma pasta estará na pasta pública.

Observe que as pastas de conteúdo são apenas compartimentos virtuais para vincular o conteúdo. Caso um conteúdo seja colocado em duas pastas, isso significa que o arquivo de conteúdo é sempre um único arquivo, mas vinculado a várias pastas. Assim, caso o conteúdo seja atualizado pelo autor 1 personalizado com acesso à pasta 1 personalizada, o mesmo conteúdo atualizado também será refletido na pasta 2 personalizada acessada pelo autor 2.
...
O conteúdo pode ser desvinculado de uma pasta especificada **SOMENTE** se também estiver associado a outras pastas. Se o conteúdo que está sendo desvinculado estiver associado apenas a uma pasta, é recomendável usar a operação MOVER.
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    ...
    <p><b>OBSERVAÇÃO:</b> Somente o conteúdo HTML do Captivate, do Presenter Video Express ou do Presenter pode ser editado.</p></td>
  </tr>
 </tbody>
</table>
...
No Adobe Learning Manager, a biblioteca de conteúdo também mantém versões do seu conteúdo carregado. Caso faça alguma alteração no conteúdo, como alterar uma apresentação do PowerPoint e carregar a apresentação novamente na biblioteca, o número da versão será incrementado em um. Isso ajuda a acompanhar as alterações no conteúdo. Uma nova versão do conteúdo pode ser aplicada a todos os objetos de aprendizado simultaneamente ou você pode aplicar atualizações individuais a cada curso.
+++
